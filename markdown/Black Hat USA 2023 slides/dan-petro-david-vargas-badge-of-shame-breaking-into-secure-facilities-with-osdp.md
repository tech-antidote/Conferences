...
T H E E L V I S H W O R D F O R F R I E N D
...
L E T ' S J U S T T A K E A L O O K
...
## **Undisclosed Location in Santa Fe, NM**
...
General: MAC is computed for and appended only to messages whose SEC_BLK_TYPE is SCS_15,
SCS_16, SCS_17, and SCS_18.,The AES algorithm is applied in CBC mode using S-MAC1 as the key
for all blocks, except the last one, and using S-MAC2 as the key for the last block. If the message
contains only one block, then only S-MAC2 is used.
ICV values: The ICV is initialized during the Secure Connection Sequence by the PD and is passed to
the CP during SCS_14 in reply osdp_RMAC_I.
...
After iti P-SC setup, in order\to reduce the message size and transmission time overhead]
the messages will contain only a partial MAC.)For méssages-whese-SEC-BLK_ TYPE 1S SCS_15,—
StS_I6, SCS_I7, and SCS_18 only the first four bytes of the computed MAC are sent. The MAC
verification will locally generate the full MAC[16] and compare the actual bytes that were received.
© Bishop Fox. All rights reserved worldwide. se
```
...
© Bishop Fox. All rights reserved worldwide. 73

## Slide 74

© Bishop Fox. All rights reserved worldwide. 74
...
Y O U S U P P O R T E N C R Y P T I O N . . . I S U P P O R T E N C R Y P T I O N . . . L E T S D O S O M E T H I N G A B O U T T H A T
...
Y E A O K , L E T S P A R T Y . **"Secure" Channel - Connection Sequence: SCS_12**

RND.B[8] C.Cryptogram PD’s cUID
...
Y O U S E E M L E G I T **"Secure" Channel - Connection Sequence: SCS_13**
...
Y O U S E E M L E G I T A S W E L L **"Secure" Channel - Connection Sequence: SCS_14**
...
The successful completion of the synchronization sequence SCS_11 through SCS_14 confirms that
...
```text
WAIT A SECOND
"Secure' Channel - Connection Sequence:SCS_15&SCS 16
D.3.2.1 $CS_15 CP->PD
The DATA field is sent in plain text (unencrypted)
Note: this form provides Message Authentication, ibut does not contain encrypted DATA)
D.3.2.2 $CS_16 PD->CP
The data field is sent in plain text (unencrypted)
...
L O O K , B U T Y O U C A N T T O U C H
...
```text
Protocol WTF #3 i.
We never Said wed encrypt
AL\ ot The data...
```

## Slide 122

P R O T O C O L W T F # … ?
...
E N C R Y P T I N G W I T H W E A K K E Y S

## **Using weak keys can't possibly backfire…**

Key 1 = 0x00, 0x00, 0x00… Key 2 = 0x01, 0x02, 0x03… Key 3 = 0x30, 0x31, 0x32… Key 4 = …
...
## **I've set everything up securely, I'm not affected by any of this**
...
A D V I C E F O R D E F E N D E R S
