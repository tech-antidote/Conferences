---
title: "Lemons and Liability Cyber Warranties as an Experiment in Software Regulation"
speakers: ["Daniel Woods"]
conference: "Black Hat"
conference_full: "Black Hat USA 2023"
edition: "USA"
year: 2023
source_pdf: "Black Hat USA 2023 slides/Daniel Woods_Lemons and Liability Cyber Warranties as an Experiment in Software Regulation.pdf"
pages: 35
sha256: "53fcd19ffac259bd6711de9984c4f8b68697ba23756330a334d241806d675e70"
text_chars: 9841
ocr_pages: 5
has_ocr: true
companion_files: []
extractor: "pymupdf4llm 1.28.2 + tesseract"
converted_at: "2026-08-11T21:15:19Z"
---
# Lemons and Liability Cyber Warranties as an Experiment in Software Regulation

**Speakers:** Daniel Woods  
**Conference:** Black Hat USA 2023  
**Source:** `Black Hat USA 2023 slides/Daniel Woods_Lemons and Liability Cyber Warranties as an Experiment in Software Regulation.pdf` (35 pages)


## Slide 1

## Lemons and Liability Cyber Warranties as an Experiment in Software Regulation

#### **Daniel W Woods**

#BHUSA   @BlackHatEvents

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
pifek hat
LUISA &
AUGUST 9-10, 20253
BRIEFINGS
Lemons and Liability
Cyber Warranties as an Experiment in
Software Regulation
4eX. THE UNIVERSITY
RAY of EDINBURGH
#BHUSA @BlackHatEvents
Daniel W Woods ge Coalition
```

## Slide 2

### **What to expect from this talk**

+ LAW
+ SOFTWARE SECURITY

Source: xkcd

#BHUSA  @BlackHatEvents

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
2)
black hat
USA 20253
What to expect from this talk
WHATCHYA DOING?
) ECONOMICS.
+ LAW
+ SOFTWARE SECURITY
Source: xkcd
```

## Slide 3

### **Agenda**

1. Convince you that **software liability is important** 2. Tell you a **story about cyber warranties**

3. Collect your esteemed perspectives on **how to design a safe harbor**

#BHUSA  @BlackHatEvents

## Slide 4

“the only two products not covered by product liability today are **religion** and **software** ”

**<u>Cybersecurity as Realpolitik (27:08)</u>**

#BHUSA  @BlackHatEvents

## Slide 5

### **Software security as a lemons market**

**cost $200k** — security reviews throughout dev lifecycle, bug bounty program etc

**cost $100k** —  none of the above

or ?

**If the buyer cannot identify insecure software,** do they buy

Akerlof, 1970. The Market for "Lemons": Quality Uncertainty and the Market Mechanism, _T he Quarterly Journal of Economics_ , Oxford University Press, vol. 84(3), pages 488-500

#BHUSA  @BlackHatEvents

## Slide 6

**How can we** **incentivize vendors to build s instead of s?**

#BHUSA  @BlackHatEvents

## Slide 7

“the only two products not covered by product liability today are **religion** and **software** , and software should not escape for much longer”

**<u>Cybersecurity as Realpolitik (27:08)</u>**

#BHUSA  @BlackHatEvents

## Slide 8

**If the vendor is liable for $1m for any security breach, does the vendor build**

  or  ?

#BHUSA  @BlackHatEvents

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
® SS A > . : y ' 4 = ‘s | .
black hat ~ i <serhs )  a -
ae ii ly ‘a ,
: ey
USA 20253
for any
security breach, does the vendor build
< aed
or wy?
```

## Slide 9

### **The right amount of software liability**

###### **No liability**

**Optimal liability**

###### **No innovation**

📉

Stricter liability regime

#BHUSA  @BlackHatEvents

## Slide 10

“We must begin to **shift liability** onto those entities that **fail to take reasonable precautions** to secure their software ”

<u>https://www.whitehouse.gov/wp-content/uploads/2023/03/National-Cybersecurity-Strategy-2023.pdf</u>

#BHUSA  @BlackHatEvents

## Slide 11

### **A** **safe harbor protects vendors from liability**

#BHUSA  @BlackHatEvents

## Slide 12

### **Safe harbor in the context of software liability**

###### Vendors are **immune from liability** if they implement:

1. Secure software development process A

2. Secure software development process B

- …

- N. Secure software development process X

#BHUSA  @BlackHatEvents

## Slide 13

### **Agenda**

2. Tell you a **story about cyber warranties**

#BHUSA  @BlackHatEvents

## Slide 14

### **The first cyber warranty**

**Black Hat USA 2014**

Jeremiah Grossman announces that WhiteHat **will pay up to $250k** in breach related costs to any* customer that is hacked

**RSA 2015**

**Limit upped to $500k** and he talks about emerging insurance market

<u>No More Snake Oil: Why InfoSec Needs Security Guarantees</u>

***T&Cs apply**

#BHUSA  @BlackHatEvents

## Slide 15

### **Competitors do not offer warranties**

###### **<u>Gartner’s Application Security Testing Market (as of 2023):</u>**

|**Vendor**|**Rating**|**Reviews**|
|---|---|---|
|**Veracode (Verracode)**|**4.7**|**304**|
|**Checkmark (SAST)**|**4.5**|**282**|
|**Rapid7 (InsightAppSec)**|**4.3**|**189**|
|**Port Swigger (Burp Suite Pro)**|**4.8**|**186**|
|**Qualys (Web App Scanning)**|**4.3**|**164**|
|**Invicti (Acunetix)**|**4.6**|**142**|
|**Synopsys (WhiteHat DAST)**|**4.5**|**142**|
|**Contrast Security (Security Platform)**|**4.6**|**137**|
|**… (No AsTech)**|#BHUS|A  @BlackHatEvents|

## Slide 16

###### **<u>Gartner’s Endpoint Protection Platforms (EPP) Market:</u>**

### **Vendors get religion**

Vendors get religion
Vendor (Product) Rating Reviews
Trellix (Endpoint Security) 4.5 1598
Symantec (Endpoint Protection) 4.4 1568
Microsoft (Defender) 4.4 1307
Sophos (Intercept X) 4.8 1118
Trend Micro (Apex One) 4.6 1052
SentinelOne (Singularity) 4.8 949
CrowdStrike (Falcon) 4.8 846
Malwarebytes (Endpoint Protection) 4.6 678
Cylance (PROTECT) 4.6 508
VMware (Carbon Black Cloud) 4.6 357

#BHUSA  @BlackHatEvents

## Slide 17

### **Vendors offering warranties as of Q1 2023***

|**Vendor**|**Rating**|**Year**|**Limit**||
|---|---|---|---|---|
|**WhiteHat Security**|**Security audit**|**2015**|**$500k**||
|**SentinelOne**|**End-point**|**2016**|**$1m**||
|**MyDigitalShield**|**Network**|**2016**|**$50k**||
|**Cymmetria**|**Deception**|**2016**|**$1m**||
|**AsTech**|**Security audit**|**2017**|**$5m**||
|**CrowdStrike**|**End-point**|**2018**|**$1m**||
|**Cybereason**|**End-point**|**2020**|**$1m**||
|**ThreatAdvice**|**MSP**|**2020**|**$250k**||
|**Deep Instinct**|**End-point**|**2021**|**$3m**||
|**Rubrik**|**Back-up**|**2021**|**$5m**||
|**Arctic Wolf**|**SOC**|**2021**|**$1m**||
|**Sophos**|**End-point**|**2022**|**$1m**||
|**Kroll**|**End-point**|**2022**|**$1m**|*Let me know if I missed any!|
|**Veam**|**Back-up**|**2023**|**$5m**||

#BHUSA  @BlackHatEvents

## Slide 18

# Do cyber warranties solve the **lemons market** ?

#BHUSA  @BlackHatEvents

## Slide 19

### **Criticisms of warranties**

**Objection 1** **Better ways to spend, such as improving security**

**Objection 2 Warranties are** **sales tricks, T&Cs always apply**

#BHUSA  @BlackHatEvents

## Slide 20

### **Objection 1: Better ways to spend money**

Answer: Cyber warranties are **costly signals** designed to escape the lemons market.

#BHUSA  @BlackHatEvents

## Slide 21

### **Warranties as a quality signal**

Cyber warranties are offered alongside **23% of all end-point protection products** for which Gartner collect ratings.

Buyers report **higher satisfaction** with cyber warrantied EPP products ( **4.81 vs 4.46** ).

#BHUSA  @BlackHatEvents

## Slide 22

### **Objection 2: Cyber warranties are** **sales tricks!**

**Answer: It depends** on the specifics of the T&Cs

#### **Yes**

**No**

Liability limits can be **deceptive** :

- $5m limit falls to $250k if customer holds 500 terrabytes or less of data

- EDR vendors often limit liability to a max of $1k per machine

- There are **no silver bullets** for security ● App testing firm’s warranty only covers CVEs known at time of audit

   - **0-days not covered**

   - ● End-point warranties typically require configuration and maintenance

#BHUSA  @BlackHatEvents

## Slide 23

##### Customer **must follow the [Vendor’s] security best practices** … includes the following:

###### Data Encryption

- Data-at-rest and in-transit are **_always encrypted_**

- **_Secure protocols_** for third-party systems

###### Application Access

- Create **_IP whitelisting_** that limits connections to Customer owned networks only

- **_SSL-certificate security_** for User Interface (UI) and

- APIs

###### API Security

- **_Secure service accounts_**

###### Data Health

- **_Back-ups are successful_** and meet the SLA

- Policies

- Retention lock is enabled for the **_Customer data_** in the SLA Policies

###### User Access

   - **_Multi-factor authentication_** for all user accounts

   - SSH key-based with **_passphrase protected keys_** for CLI authentication

   - User roles are assigned with **_least privilege access_**

- Scoped **_API roles with least privilege_**

#BHUSA  @BlackHatEvents

## Slide 24

### **Cyber warranties cliff notes**

- Since the first warranty at BlackHat 2024, an extra **15 cyber warranties have been announced**

- **23% of EPP products** now offered with a warranty

- Warranties **signal effectiveness** of InfoSec products

- Pay attention to **terms and conditions**

   - Some sales tricks

   - Some incentives for better security

#BHUSA  @BlackHatEvents

## Slide 25

### **Agenda**

###### 3. Collect your esteemed perspectives on **how to design a safe harbor**

#BHUSA  @BlackHatEvents

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
bisek hat
USA 20253
Agenda
SAMPLING BIAS
BYES, | LOVE RESmNDI NG,
TO SURNEYS
DB NO, 11055 THEM in THESIN
99:3) a iQ
0-2
“WE RECEIVED S500 RESPONSES AND
FOUND THAT PEOPLE LOVE RESPONDING
To sveNENSs"
Sketchplanati ons
3. Collect your esteemed perspectives on how to design a safe harbor
```

## Slide 26

# What is the **design space** in this context?

#BHUSA  @BlackHatEvents

## Slide 27

### **Broad options**

higher level, less specific

List of measures that Frameworks a vendor qualify vendor for safe must follow to qualify for harbour safe harbour

Guiding principles that vendors must follow (e.g. secure by design)

#BHUSA  @BlackHatEvents

## Slide 28

### **Safe harbor vs** **reverse safe harbor**

- Not liable if you do: 1. Good practice 1 2. Good practice 2 3. …

- Become liable if you do: 1. Bad practice 1 2. Bad practice 2

   3. …

<u>Idea from Derek Bambauer</u>

#BHUSA  @BlackHatEvents

## Slide 29

### **Which areas of security?**

Appsec

and/or

Infosec

#BHUSA  @BlackHatEvents

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
2)
black hat
USA 20253
Which areas of security?
DETECT
RECOVER
Appsec and/or Infosec
```

## Slide 30

### **One vs many safe harbors**

**Online gaming**

Healthcare

Stricter liability regime

#BHUSA  @BlackHatEvents

## Slide 31

# What about **open source** software ?

#BHUSA  @BlackHatEvents

## Slide 32

# **Untangling root causes** in complex systems?

#BHUSA  @BlackHatEvents

## Slide 33

# **Legal evidence** ?

#BHUSA  @BlackHatEvents

## Slide 34

### **Key takeaways**

**Liability pushes vendors to secure software**

**Vendors are increasingly liable for security failures**

**Cyber warranties achieve higher customer satisfaction**

**Designing the safe harbor is an important technical problem**

#BHUSA  @BlackHatEvents

## Slide 35

### Thank you!

Please share the survey with relevant colleagues and reach out to share your ideas and experiences.

**Contact details** <u>daniel.woods@ed.ac.uk daniel.woods@coalitioninc.com</u> **~~Big QR code of survey.~~**

**BIG QR CODE THAT LINKS TO THE SURVEY**

**(on day of presentation)**

<u>https://www.danielwoods.info/</u>

#BHUSA  @BlackHatEvents
