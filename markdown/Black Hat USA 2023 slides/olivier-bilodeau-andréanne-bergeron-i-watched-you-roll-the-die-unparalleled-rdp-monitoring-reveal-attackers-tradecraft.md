---
title: "I Watched You Roll the Die Unparalleled RDP Monitoring Reveal Attackers' Tradecraft"
speakers: ["Olivier Bilodeau", "Andréanne Bergeron"]
conference: "Black Hat"
conference_full: "Black Hat USA 2023"
edition: "USA"
year: 2023
source_pdf: "Black Hat USA 2023 slides/Olivier Bilodeau & Andréanne Bergeron_I Watched You Roll the Die Unparalleled RDP Monitoring Reveal Attackers' Tradecraft.pdf"
pages: 64
sha256: "fa1667e4f7bc04d017cd021afb78293b2089b3546bc7bd2ed9fa65af0984edaa"
text_chars: 15701
ocr_pages: 18
has_ocr: true
companion_files: []
extractor: "pymupdf4llm 1.28.2 + tesseract"
converted_at: "2026-08-11T21:23:14Z"
---
# I Watched You Roll the Die Unparalleled RDP Monitoring Reveal Attackers' Tradecraft

**Speakers:** Olivier Bilodeau, Andréanne Bergeron  
**Conference:** Black Hat USA 2023  
**Source:** `Black Hat USA 2023 slides/Olivier Bilodeau & Andréanne Bergeron_I Watched You Roll the Die Unparalleled RDP Monitoring Reveal Attackers' Tradecraft.pdf` (64 pages)

## Slide 1

## I Watched You Roll the Die: Unparalleled RDP Monitoring Reveal Attackers Tradecraft

Olivier Bilodeau & Andréanne Bergeron, GoSecure

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
pifek hat
USA &
AUGUST 9-10, ©2025
BRIEFINGS
I WATCHED Y®U ROLL THE DIE:
UNPARALLELED RDP IMM@NITO®RING
REVEAL ATTACKERS TRADECRAFT
Olivier Bilodeau & Andréanne Bergeron, GoSecure
```

## Slide 2

###### Disclaimer: Dungeons and Dragons Nerdiness

#BHUSA @BlackHatEvents

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
+) _ DISCLAIMER: DUNGE@NS
bIBEKh|E AND DRAGONS NERDINESS
WE iN
A eARAIG
RAG ws INS
ON Ly IN THEATRES MARCH 31
‘feds =, ONS
```

## Slide 3

#### Who are We?

###### Andréanne Bergeron

- Ph.D in criminology, Olivier Bilodeau

- **Université de Montréal**

   - Cyber se curity Research Director, **GoSecure**

- Cybersecurity Researcher, **GoSecure**

   - President, **NorthSec** Training sessions, Conference and CTF in Montréal

- Co-VP Engagement and Outreach, **NorthSec**

- Scientific Director, **OpenSource Research Group**

- Co-founder of **MontréHack** Applied Security Workshops

- Enthusiastic Serial Speaker

#BHUSA @BlackHatEvents

## Slide 4

The Quest

#BHUSA @BlackHatEvents

## Slide 5

###### The Quest (Agenda)

- RDP Interception Explained

- 5 Attacker Classes

   - Rangers, Thieves, Barbarians, Wizards and Bards

- The Arsenal they use

- Some Memorable Session Highlights

#BHUSA @BlackHatEvents

## Slide 6

### Call to Adventure

#BHUSA @BlackHatEvents

## Slide 7

#### Today’s Game

- Ransomware is a problem, RDP is an important factor

- Let’s Leverage Unparalleled RDP Capture Depth

#BHUSA @BlackHatEvents

## Slide 8

###### Remote Desktop Protocol

#BHUSA @BlackHatEvents

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
pifhnar REMOTE DESKTOP
USA 2&O0es3 PROT@®COL
) my-server.gosec.co - Remote Desktop Connection
i Server Manager
© ~ Server Manager * Local Server Manage Tools View
fm_~=~PROPERTIES
® Remote Desktop Connection BRLocaliserver
ig All Servers
Remote Desktop if File and Storage Services >
~o2s) Connection
Computer: [fe ample: computer fabrikam.com
Username: None specified
The computer name field is blank. Enter a full remote computer
name.
¥) Show Options
EC2AMAZ-6IS509) 10016 Error Microsoft-Windows-DistributedCOM System 3/24/2022
EC2AMAZ-61S509J 10016 Error Microsoft-Windows-DistributedCOM System 3/24/2022 5:24:18 PM
```

## Slide 9

###### PyRDP: Tool Characteristics

###### **4 years in development**

**RDP Monster-in-the-Middle        (MITM)**

- RDP Interception

- Records Everything

- RDP Tampering Engine

###### **RDP Player**

- Real-time viewing and takeover

- Replay of previous captures

#BHUSA @BlackHatEvents

## Slide 10

###### The PyRDP Crystal Ball

#BHUSA @BlackHatEvents

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
4a Foon awe *
7 _.
:
biSekhat THE PYRDP CRYSTAL : <
USA 20253 BALL
#BHUSA @BlackHatEvents
```

## Slide 11

#BHUSA @BlackHatEvents

## Slide 12

### Basically, surveillance cameras for RDP Systems

#BHUSA @BlackHatEvents

## Slide 13

###### RDP Honeypot with PyRdp

Science!

#BHUSA @BlackHatEvents

## Slide 14

###### PyRDP: Learn More

###### Source Code / Documentation

- <u>https://github.com/GoSecure/pyrdp</u>

- <u>PyRDP ReadMe</u>

- <u>PyRDP Transparent Proxying Guide</u>

- <u>Windows RDP Certificate Extraction</u>

Past Presentations & Blogs

   - <u>Introduction Blog Post</u>

   - <u>NorthSec 2019 Talk</u>

   - <u>BlackHat Arsenal 2019</u>

   - <u>Blog: PyRDP on Autopilot</u>

   - <u>DerbyCon 2019 (Video)</u>

- <u>RDP Connection Sequence</u>

- <u>RDP Basic Protocol Specification</u>

- <u>DEFCON 28 Demo Labs</u>

- <u>Blog: Announcing PyRDP 1.0</u>

- <u>1.0 released at SecTor 2020</u>

- <u>BlackHat Arsenal 2021</u>

- <u>Purple RDP: Red and Blue Tradecraft</u>

- <u>Blog: 1.2 release "Rudolph Desktop Protocol"</u>

#BHUSA @BlackHatEvents

## Slide 15

Welcome to the
Dungeon,
Adventurers

#BHUSA @BlackHatEvents

## Slide 16

###### Collected intelligence

#BHUSA @BlackHatEvents

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
USA 20253 INTELLIGENCE
World: RDP Sessions Captured
GREENLAND ™ Total number of events
° LAYERS GO >
Net-NTLMv2 Hashes Captured
ICELAND RUSSIA
FINLAND
CANADA NORWAY
DENMARK
80 e
IRELANS PAARG
43)
104 UKRAINE
FRANCE aA ane KAZAKHSTAN MONGOLIA ee Successful Logins
34) 2 BULGARIA ieeeaak “ :
- S We GREECE TURKEY TAJIKISTAN SOUTH
a 729) CHINA REA
NA rag AFGHANISTAN es
RVA EGYPT ERABIA
TAIWAN
meaco una 2.65k
4 MAURITANIA OMAN e
y MALL nee: YEMEN MYANMAR
wy SENEGAL CHAD “SUDAN tw ViETNAM
GUINEA :
130 sour Bonn Files transferred
GUYANA UBERIA CAMEROON on
COMBIA
ECUADOR DEMOCRATIC EAP
REPUBLIC OF =
a Pr PAPUA NEW
PERU THE CONGO 8] wo GUINEA
BRAZH. ANGOLA =
ZAMBIA
BOLIVIA
MOZAMBIQUE
PARAGUAY NAMIBIA,
Bll AUSTRALIA
Sessions Captured
ARGENTINA
a
@
lat: 71.4187, lon 5, zoom: 188
2.39K
lastic Maps Service (7, OpenMapTiles (4, OpenStreetMap contributors (7
```

## Slide 17

Collected intel (cont.)

2390
recorded
95
sessions
unrelated
attackers
455
sessions
with
content

339 sessions of interest

#BHUSA @BlackHatEvents

## Slide 18

###### Att&ck indicators

**T1070  Indicator T1082 System T1219 Remote Access T1090 Proxy Removal Information Discovery Software T1217 Browser T1005 Data from Local Information Discovery System T1562 Impair Defenses T1059 Command and T1496 Resource Scripting Interpreter Hijacking T1098 Account Manipulation T1595.003 Active Scanning: Wordlist Scanning T1016 System Network Configuration Discovery T1595.001 Active Scanning: Scanning IP Blocks**

#BHUSA @BlackHatEvents

## Slide 19

Characterizing

Attackers’ D&D

Style

#BHUSA @BlackHatEvents

## Slide 20

#BHUSA @BlackHatEvents

## Slide 21

#BHUSA @BlackHatEvents

## Slide 22

## Rangers

- Explore the system

- Run reconnaissance

- Might be evaluating the system for others

#BHUSA @BlackHatEvents

## Slide 23

##### Ranger In Action

#BHUSA @BlackHatEvents

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
biSekhat RANGER IN ACTION a FON
USA 20es3
| Hostname: EC2AMAZ-31J15JO
Instance ID; i-01312d4a89eeceeS5
Public IPv4 Address: 3.88.186.135
Private IPv4 Address: 172.31.50.210
Instance Size; t2.micro
Q Availability Zone: us-east-1d
- Architecture: AMD64
Recycle Bin
2
eee Total Memory: 1024 MB
Network Performance: Low to Moderate
EC2 Word
Micros...
11:41 AM
```

## Slide 24

#BHUSA @BlackHatEvents

## Slide 25

## Thieves

- Monetize the RDP access

- Examples

   - Traffmonetizer

   - Traffic generator

   - Cryptominers

#BHUSA @BlackHatEvents

## Slide 26

Thieves In Action

#BHUSA @BlackHatEvents

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
bisek hat
USA 20es3
DTRAFFMONETIZER
Log In
Chat with us eo
|
```

## Slide 27

#BHUSA @BlackHatEvents

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
bi | k a t | Hostname: EC2AMAZ-OJK3L78
ac a Recycle Bin ChromeSetup Instance ID; i-O49df4cOb0dfa528c
USA 2&0e3 236.56
31.32.40
Share
4h resources > miners win32 > xmrig-6,18.0 v
a)
Q
a
EC2 Feedback in
Name Date modifie Type e
wr Quick access a conpmsannrnlll vv to Moderate
enchmar 1P
Gl Desktop - 7
benchmark_10M 1PM V Y KE
g Downloads : 3 pte omer ere -
config.json 1P i File
EC2 Microsoft =| Documents [&) pool_mine_example 22 7:31 PM V Y KB
Windows G... =) Pictures es rtm_ghostrider_example 7:31 PM ~
2 d SHA256SUMS 5/202 1PM File
= admins =
4 solo_mine_example 7:31 PM £
[z=] Contacts —
a] start #2022 #33 ows Co B
Google El Desiaop WinRingOx64.sys 15/2022 7:31PM System file 5 KB
Chrome =| Documents & xmrig i 11/15/2022 7:31 PM B
4 Downloads
Xr Favorites
@ Links
ded d Music
=) Pictures
#P Saved Games
u n Searches
unMineable Ba Videos
Miner (M...
GS This PC
U a 10 items
Gl
unMineable
Miner (MFI)
4:03 PM
Fel de 3/6/2023 =
```

## Slide 28

#BHUSA @BlackHatEvents

## Slide 29

## Barbaria ns

- Brute-force more computers

- Work with lists of IP addresses, usernames and passwords

#BHUSA @BlackHatEvents

## Slide 30

###### Barbarian In Action

#BHUSA @BlackHatEvents

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
Q |
blackhat BARBARIAN IN ACTI@N
USA 2&0e253
Hostname: EC2AMAZ-PH85V08
Instance ID: i-07445ee7ec1a9a5f9
© res://iesetup.dil/HardAdmin.ht O ~ G | G internet Explorer Enhanced ... Public IPv4 Address: 54.237.89.231
Private IPv4 Address: 172.31.40.93
Internet Explorer Enhanced Security Configuration is enabled Instance Size: t2.micro
Intern - buoiability Zone: us-east-le
defin Application T MasScan ure: AMD64
migh ara
Secu Share fiew Manage y: 1024 MB
This ; formance: Low to Moderate
pau | This PC Downloads MasScan MasScan
funct
infor
ge Quick access
Desktop
} Downloads
_config
exclude
Input
= Documents IPs
= Pictures [i] masscan
MI Massscan_GU!
msvcr100.dil
> Network Output
Packet.dil
(@ winpcap-4.13
Gd This PC
wpcap.dil
11 items 1 item selected 423 KB
5 items
```

## Slide 31

#BHUSA @BlackHatEvents

## Slide 32

## Wizards

- Use the RDP access as a portal to connect to other computers

- Show a high level of skill

#BHUSA @BlackHatEvents

## Slide 33

##### Wizard in action

#BHUSA @BlackHatEvents

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
piftxhat WIZARD IN ACTION
USA 20es3
Hostname: EC2AMAZ-LTPPV2V
Instance ID: i-0b2165a3133c1e072
Public IPv4 Address: 18.212.55.169
Private IPv4 Address: 172.31.58.15!
Instance Size: t2.micro
Availability Zone: us-east-1d
&
Architecture: AMD64
L Remote Desktop
*<. Connection Total Memory: 1024 MB
Network Performance: Low to Mod
Computer: xample?
Username: None specified
‘The computer name field is blank. Enter a full remote computer
name
4:37PM
```

## Slide 34

#BHUSA @BlackHatEvents

## Slide 35

## Bards

- No apparent hacking skills

- Examples:

   - Google searches related to hacking

   - Porn watching

   - Downloading movies

- Might have bought the access

#BHUSA @BlackHatEvents

## Slide 36

# Bards In Action

#BHUSA @BlackHatEvents

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
& Q Search with Google o? enter address 9 =
3) Import bookmark
blackhat @ Youtube
USA 20es3 outubecom 6
Ei Facebook
facebook.com
W Wikipedia
kipedia.org
Reddit
reddit.com
Twitter
twitter
T
G
a fl
Amazon Hotels.com
Sponsored Sponsored
YouTube Facebook
-
Wikipedia Reddit
```

## Slide 37

# Why porn?

Internet censorship → Ban on pornography

#BHUSA @BlackHatEvents

## Slide 38

###### Sessions by profiles

###### **Number of RDP sessions associated with the profiles**

Wizards
Bards
Thieves
Barbarians
Rangers
0 20 40 60 80 100 120 140 160 180

#BHUSA @BlackHatEvents

## Slide 39

###### Attackers’ Overlap

10
Thieves
8
12
1
21 Barbarians
Rangers
82
127
Wizards
2
1 3
2
2
3 6
Bards

#BHUSA @BlackHatEvents

## Slide 40

Attackers’

Weaponry

#BHUSA @BlackHatEvents

## Slide 41

###### xDedic RDP Patch

Used By

###### ATT&CK

- T1098

- T1136.001

- T1543

- T1562.004

- T1556

- T1078

```
daddc833bffcade36b432b21046487b2
```

```
9dcd2a162d91b503334a52caee9c1fd2
```

###### SHA256-Hash

- Persistence mechanism

- Removes restrictions on non-server Windows

- Group taken down but tool still exists

Description

#BHUSA @BlackHatEvents

## Slide 42

#### NLBrute

Used By

ATT&CK

• T1133
• T1078
• T1595
• T1110

- `4e0060e367f024dae16928fce3adffa79ea0ff85ac595125b3e769efc02a58af`

- `1250abd2ac13d7febd8b62e8d5ec4db15ec43ff88dd2ad10ba85130869d800be`

- `5187c0f887003f72bd676d0d1149c5cbcc1a69258b4bb660473dcbc170b41f0f`

SHA256-Hash

- Multi-threaded brute-forcing tool dedicated to RDP

- Russian author arrested and extradited to the USA on Feb 23<sup>rd</sup> , 2023

Description

#BHUSA @BlackHatEvents

## Slide 43

#### Masscan GUI

Used By

ATT&CK

   - T1133

   - T1078

   - T1595

   - T1110

- `de903a297afc249bb7d68fef6c885a4c945d740a487fe3e9144a8499a7094131`

- `f7f2af6f4dd9d0af03e6af3aba685969ec9ed22295055882e571bef3e96bbc4e`

###### SHA256-Hash

- Well known tool, they use GUI wrappers

- Advanced Mode variant

Description

#BHUSA @BlackHatEvents

## Slide 44

#### SilverBullet

Used By ATT&CK

• T1595.001
• T1090
• T1119
• T0814
• T1110.004
• T1071.001
TODO

SHA256-Hash

- Proxy-first scriptable Web “testing”

- Pre-made configuration kits

- Clearly little legitimate use Description

#BHUSA @BlackHatEvents

## Slide 45

###### SilverBullet (cont.)

From their website:

_OpenBullet is a webtesting suite that allows to perform requests towards a target webapp and offers a lot of tools to work with the results. This software can be used for_ **_scraping_** _and_ **_parsi_** **_ng data_** _, automated_ **_pentesting_** _, unit testing through_ **_selenium_** _and much more._

#BHUSA @BlackHatEvents

## Slide 46

###### SilverBullet (cont.)

Again, on their website:

**_IMPORTANT!_** _Performing (D)DoS attacks or credential stuffing on sites you do not own (or you do not have permission to test) is_ **_illegal!_** _The developer will not be held responsible for improper use of this software._

#BHUSA @BlackHatEvents

## Slide 47

tools

#BHUSA @BlackHatEvents

## Slide 48

###### Windows Defender Remove Script

Used By

ATT&CK

• T1562
• T1518

5809c8109a48980fef1f216cc37e6aba
78cab7a75414e2687f263a539f033908
SHA256-Hash

- Leverages “install_wim_tweak.exe”

- Unhides Microsoft OS packages

- Removes Defender

Description

#BHUSA @BlackHatEvents

## Slide 49

###### dControl (aka DefenderControl)

Used By

ATT&CK

• T1562

   - T1518

- `6606d759667fbdfaa46241db7ffb4839`

d2c47b88a20120446f41e916cad77d0b

SHA256-Hash

- Disables Defender and unload its driver

- Documented by The DFIR Report

Description

#BHUSA @BlackHatEvents

## Slide 50

###### Gammadyne Mailer

Used By ATT&CK
• T1598
• T1566

`1883b2ab48321bdf58de973063f5a700 cfd4f39bd3b483efe314cc514ec777e3` SHA256-Hash

Phishing, spearphishing and spamming tool

Description

#BHUSA @BlackHatEvents

## Slide 51

###### Tool classification by objectives

Fraud InfoStealer Recon
2% 1% 2%
Proxy
2%
Protection
3%
Scanning and  Monetization
attack
9%
48%
Download
9%
Communication
11%
Enabler
13%

#BHUSA @BlackHatEvents

## Slide 52

#### And more…

Thieves

- XMRig (cryptominer)

- • Monetizing browsers (Websidia, Netbox)

- Traffmonetizer

- WorldWind

- AlexaMaster

- Honeygain dashboard

###### Rangers

- Change pass one click

###### Miscellaneous

- Telegram Bot API toolkit

- IPVanish

Barbarians

   - KPort Scan

   - NLA checker

   - Mimikatz

   - Advanced IP Scanner

   - STORM

   - OK Prog Proxy leecher

   - Spotify proxyless brute checker

   - • Soft Perfect Scanner or netscanold

   - • RDP Cracked by Arman

   - Brute XML

- Time restart reloaded (Chrome extension)

- Backset

#BHUSA @BlackHatEvents

## Slide 53

# Crystal Ball

Showing a Range of Attackers’ Skills (or lack thereof) at Work

#BHUSA @BlackHatEvents

## Slide 54

# Attackers might work in team

#BHUSA @BlackHatEvents

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
gQ _) Scale to window
black hat
USA 2023 —
s6.picofile.com,
ue sobT g Guls soLT yLSsl (¢Las - PicoFile.com’
NLBrute 1.2.zip
sagbils SLY eyeT OL 4) gly laid (inl) Cug> oligS Kid
yon.ir/nibrute12
econ © Cy ee
CQ 583 207 esis cork Be anal Wp] @
pelo
dgbils Sid ob ,er% aul |asVoo OTF
ADSL cee pw pcs ss}
https://s6.picofile.com/d/8266183492/228df8aa-3e7e-4fac
(aguilei wg> Gull Si GS wd
sobls 4
Yoga 43.0 re
2 Olibill go> aie odio bl) &
```

## Slide 55

Some attackers do not speak English

#BHUSA @BlackHatEvents

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
bisek hat
USA 20es3
onsible for Google Ads costs and how they pay. Le:
Jit or debit card
a
24 784|
```

## Slide 56

###### Our tool is great for intel

#BHUSA @BlackHatEvents

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
—
| wr
pifckhatr DUR TOOL IS GREAT . b Sn SAS Bi
Recycle Bin Telegram e
EC2
```

## Slide 57

###### Alphabet found in sessions

Vietnamese
Cyrillic 5%
Chinese
13%
2%
F arsi
29%
Arabic
51%

N=45

#BHUSA @BlackHatEvents

## Slide 58

End Game XP

and Treasure

The Take-Aways

#BHUSA @BlackHatEvents

## Slide 59

#### The Party

#BHUSA @BlackHatEvents

## Slide 60

#### Take aways

Blue Team: Consume the IOCs, roll out your own traps Law Enforcement: Lawfully intercept the RDP environments used by ransomware groups.

_If the attackers fear getting caught on camera, they will be more careful which will slow them down._

#BHUSA @BlackHatEvents

## Slide 61

Cloud (several providers)

Honeynet Plan
Attackers

… …

Science!

#BHUSA @BlackHatEvents

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
piseikhat “tous (several providers) . % Science! | _ ) :
oe Peal! 9st idio & H
jupyter
Ne
a
QO
elastic
| «
NV ‘Ww lle RCLONE
C1* C D fe doc ker
```

## Slide 62

##### One more thing

###### PyRDP is open source

###### Now, our FQ RDP capture parser too!

Analysis at scale

#BHUSA @BlackHatEvents

## Slide 63

See you at the

Thank You! Questions? Andréanne Bergeron <u>abergeron@gosecure.net</u>

Olivier Bilodeau <u>obilodeau@gosecure.net @obilodeau</u>

<u>https://gosecure.net/blog/</u>

<u>https://nsec.io</u>

**Artwork**

#BHUSA @BlackHatEvents

## Slide 64

###### References / IOC

#BHUSA @BlackHatEvents
