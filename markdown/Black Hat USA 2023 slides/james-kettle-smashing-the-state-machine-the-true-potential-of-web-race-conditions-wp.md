...
I'll also introduce the single-packet attack; a jitter-dodging strategy that can squeeze 30 requests sent from Melbourne to Dublin into a sub-1ms execution window.

This paper is accompanied by a full complement of free online labs, so you'll be able to try out your new skill set immediately.
...
- Case studies
   ...
   - Single-endpoint Deferred

- Further research potential

   - Partial construction

   - Unsafe data structures Single-packet attack
...
To begin, let's recap race condition fundamentals. I'll keep this brief - if you'd prefer an in-depth introduction, check out our new Web Security Academy topic<sup>1</sup> .
...
- Repeatedly applying a single discount code<sup>2</sup>

- Rating a product multiple times

- Withdrawing or transferring cash in excess of your account balance

- Reusing a single CAPTCHA solution<sup>3</sup>

- Bypassing an anti-bruteforce rate-limit<sup>4</sup>
...
The true potential of race conditions can be summed up in a single sentence. Every pentester knows that multi-step sequences are a hotbed for vulnerabilities, but _with race conditions, everything is multi-step_ .
...
POST /login 302 Found GET /role 200 Found POST /role 302 Found GET /application 200 OK
...
My primary mistake was the assumption that the GET request wouldn't change the application state. However, there's a second assumption that's even more common - that "requests are atomic". If we ditch this assumption too, we realize this pattern could occur _in the span of a single login request_ :
...
I implemented the single-packet attack in the open-source Burp Suite extension Turbo Intruder<sup>7</sup> . To benchmark it, I repeatedly sent a batch of 20 requests 17,000km from Melbourne to Dublin, and measured the gap between the startof-execution timestamp of the first and last request in each batch. I've published the benchmark scripts in the examples folder<sup>8</sup> so you can try them for yourself if you like.
...
The single-packet attack was inspired by the 2020 USENIX presentation Timeless Timing Attacks<sup>11</sup> . In that presentation, they place two entire HTTP/2 requests into a single TCP packet, then look at the response order to compare the serverside processing time of the two requests:
...
This concept is honestly pretty obvious, and after implementing it I discovered someone else had the same idea back in 2020<sup>12</sup> , but nobody noticed at the time and their algorithm & implementation didn't receive the polish, testing and integration essential to prove its true value. The reason I'm so excited about the single-packet attack is that it's powerful, universal, and trivial. Even after spending months refining it to work on all major webservers the algorithm is still so simple it fits on a single page, and so easy to implement that I expect it to end up in all major web testing tools.

The primary reason it's so easy to implement is that thanks to some creative abuse of Nagle's algorithm<sup>13</sup> , it doesn't require a custom TCP or TLS stack. You can just pick an HTTP/2 library to hook into (trust me, coding your own is not much fun), and apply the following steps:
...
- Send a ping packet to warm the local connection. If you don't do this, the OS network stack will place the first finalframe in a separate packet.
...
As a result, it's important not to attribute inconsistent request timing to application behavior such as locking mechanisms that only allow a single thread to access a resource at once. Also, front-end request routing is often done on a perconnection basis, so you may be able to smooth request timing by performing server-side connection warming - sending a few inconsequential requests down your connection before performing the attack. You can try this technique out for yourself on our multi-endpoint lab<sup>16</sup> .
...
#### **3) Will the requests affect the same record?**
...
Classic multi-step exploits can provide inspiration for race condition attacks. While testing an online shop a while ago, I discovered that I could start a purchase flow, pay for my order, and then add an extra item to my basket before I loaded the order confirmation page - effectively getting the extra item for free. We later made a replica of this vulnerability<sup>18</sup> for training purposes.

There's a documented<sup>19</sup> race condition variation of this attack that can occur when the payment and order confirmation are performed by a single request.
...
If you encounter this problem, you may be able to solve it by abusing a common security feature. Webservers often have 'leaky bucket' rate-limits which delay processing of requests if they're sent too quickly. You can abuse this by sending a large number of dummy requests to trigger the rate-limit and cause a server-side delay, making the singlepacket attack viable even when delayed execution is required:
...
I've recorded a video demonstrating the full discovery process on a remote Gitlab installation, which you can find embedded in the live version of this paper<sup>21</sup>
...
The vulnerability seemed to originate from the way Gitlab had integrated Devise<sup>22</sup> , a popular authentication framework for Ruby on Rails. I explored the Devise codebase via Confirmable.rb<sup>23</sup> , and Gitlab via their patch for my finding<sup>24</sup> . Analyzing the race condition from a white-box perspective proved quite challenging, especially around the boundary between Devise and Gitlab, but here's my best shot at explaining the inner workings of this vulnerability.
...
We've built a replica of this vulnerability<sup>25</sup> so you can practise your single-endpoint exploitation skills.
...
1. https://portswigger.net/web-security/race-conditions

2. https://portswigger.net/web-security/race-conditions/lab-race-conditions-limit-overrun

3. https://portswigger.net/research/cracking-recaptcha-turbo-intruder-style

4. https://portswigger.net/web-security/race-conditions/lab-race-conditions-bypassing-rate-limits

5. https://portswigger.net/research/cracking-recaptcha-turbo-intruder-style

6. https://www.josipfranjkovic.com/blog/race-conditions-on-web

7. https://github.com/PortSwigger/turbo-intruder

8. https://github.com/PortSwigger/turbo-intruder/blob/master/resources/examples/

9. https://portswigger.net/web-security/race-conditions/lab-race-conditions-limit-overrun

10. https://github.com/PortSwigger/turbo-intruder/blob/master/resources/examples/race-single-packet-attack.py

11. https://www.usenix.org/conference/usenixsecurity20/presentation/van-goethem

12. https://aaltodoc.aalto.fi/bitstream/handle/123456789/47110/master_Papli_Kaspar_2020.pdf

13. https://en.wikipedia.org/wiki/Nagle%27s_algorithm

14. https://github.com/PortSwigger/turbo-intruder/blob/master/src/SpikeEngine.kt

15. https://github.com/PortSwigger/turbo-intruder/blob/master/src/SpikeConnection.kt

16. https://portswigger.net/web-security/race-conditions/lab-race-conditions-multi-endpoint

17. https://github.com/PortSwigger/turbo-intruder/blob/master/resources/examples/email-link-extraction.py

18. https://portswigger.net/web-security/logic-flaws/examples/lab-logic-flaws-insufficient-workflow-validation

19. https://soroush.secproject.com/downloadable/common-security-issues-in-financially-orientated-webapplications.pdf

20. https://gitlab.com/albinowax1

21. https://portswigger.net/research/smashing-the-state-machine#demo

22. https://github.com/heartcombo/devise

23. https://github.com/heartcombo/devise/blob/ec0674523e7909579a5a008f16fb9fe0c3a71712/lib/devise/mod els/confirmable.rb

24. https://gitlab.com/gitlab-org/gitlab/-/commit/e4d8d4f818275d42469d154b72fc6367b2b86bbb

25. https://portswigger.net/web-security/race-conditions/lab-race-conditions-single-endpoint

26. https://research.nccgroup.com/wp-content/uploads/2020/07/going-auth-the-rails-whitepaper.pdf

27. https://github.com/PortSwigger/turbo-intruder/blob/master/resources/examples/email-link-extraction.py

28. https://portswigger.net/web-security/race-conditions/lab-race-conditions-partial-construction

29. https://googleprojectzero.blogspot.com/2021/01/the-state-of-state-machines.html
