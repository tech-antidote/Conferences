---
title: "Reversing Rust Binaries One step beyond strings (workshop)"
speakers: ["Cindy Xiao"]
conference: "REcon"
...
Project Browser _[rust-one-step-beyond-strings] simple-downloader.bndb x +
#% Symbols < arch symbo! ELF* Linear High Level IL ¥
...
File Edit View Analysis Debugger Plugins Window Help
Project Browser _[rust-one-step-beyond-strings] simple-downloader.bndb x +
# Symbols © Search symbol ELF* Linear High Level IL ¥
Name
init
sub_6020
sub_6036
...
8 |
9 3} Prints to the standard output, with a newline.
On all platforms, the newline is the LINE FEED character ( \n / U+0QOA ) alone (no additional
CARRIAGE RETURN ( \r / U+O0OD )).
This macro uses the same syntax as format , but writes to the standard output instead. See
[ std::fmt ] for more information.
The println! macro will lock the standard output on each call. If you call println! within
...
```text
44
B crates.io Press 'S' to focus this searchbox... Browse All Crates Log in with GitHub
reqwest v0.12.2
higher level HTTP client library
#client #http #request
Readme 93 Versions Dependencies Dependents
...
9 . text()
10 ‘| reqwest::blocking:: response:: Response
11 pub fn text(self) — crate::Result<figsums>
12 let-p |
13 std:;| Get the response text.
14 :| This method decodes the response body with BOM sniffing and with malformed sequences replaced
...
e Inherit the current process's environment
e Inherit the current process's working directory
e Inherit stdin/stdout/stderr for [ spawn ]or[ status ], but create pipes for[ output ]
...
### `Cindy Xiao_Reversing Rust Binaries One step beyond strings (workshop)_code .txt`

```text
https://github.com/cxiao/rust-reversing-workshop-recon-2024
```
