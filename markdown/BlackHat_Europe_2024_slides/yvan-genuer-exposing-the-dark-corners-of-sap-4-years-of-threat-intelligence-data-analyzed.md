---
title: "Exposing the dark corners of SAP 4-Years of Threat Intelligence data analyzed"
speakers: ["Yvan Genuer"]
conference: "Black Hat"
conference_full: "Black Hat Europe 2024"
edition: "Europe"
year: 2024
source_pdf: "BlackHat_Europe_2024_slides/Yvan Genuer_Exposing the dark corners of SAP 4-Years of Threat Intelligence data analyzed.pdf"
pages: 78
sha256: "33fb7e08ac07015140249a6b3fe2360636623d44d2c75b0099f4de12db7c9e22"
text_chars: 17352
ocr_pages: 20
has_ocr: true
redacted_secrets: 0
ocr_confidence: 88.2
ocr_unreliable_blocks: 0
vision_verified_blocks: 1
ocr_timeouts: 0
pages_recovered_from_text_layer: 0
companion_files: []
extractor: "pymupdf4llm 1.28.2 + tesseract"
converted_at: "2026-08-12T05:02:07Z"
---
# Exposing the dark corners of SAP 4-Years of Threat Intelligence data analyzed

**Speakers:** Yvan Genuer  
**Conference:** Black Hat Europe 2024  
**Source:** `BlackHat_Europe_2024_slides/Yvan Genuer_Exposing the dark corners of SAP 4-Years of Threat Intelligence data analyzed.pdf` (78 pages)


## Slide 1

### Exposing the dark corners of SAP 4-Years of Threat Intelligence data analyzed

Speaker: Yvan Genuer

#BHEU   @BlackHatEvents

## Slide 2

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 3

87% 77% 100%
of the Global  of the world’s  of F500 Oil & Gas
2000 use SAP transaction revenue

https://www.sap.com/about/company.html#fast-facts

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 4

Information Classification: General

https://warcraft3.blizzard.com

#BHEU  @BlackHatEvents


> Recovered by OCR — confidence 94/100 on the text kept, 59/100 across the whole page. Wording is approximate. Verify exact values against the source PDF.

```text
https://warcraft3.blizzard.com
```

## Slide 5

Information Classification: General #BHEU  @BlackHatEvents


> Recovered by OCR — confidence 78/100 on the text kept, 59/100 across the whole page. Wording is approximate. Verify exact values against the source PDF.

```text
Chat (F12) ye ae
rt Quests =e Menu (F10)
```

## Slide 6

?
?
?

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 7

?
?
?

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 8

Information Classification: General #BHEU  @BlackHatEvents

## Slide 9

#### 2020 - 2023 (4 Years)

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 10

2020 - 2023 (4 Years) Varied across the open - deep - dark web

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 11

2020 - 2023 (4 Years) Varied across the open - deep - dark web

Criminal Forums Chat Sites

Ransomware Incidents Ransomware Group Sites and Chat

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 12

## The Rise of Interest on SAP

Information Classification: General

#BHEU  @BlackHatEvents


> Recovered by OCR — confidence 84/100 on the text kept, 49/100 across the whole page. Wording is approximate. Verify exact values against the source PDF.

```text
Menu (F10) Allies (F Chat (F1 2) ro) 1205 15/24
```

## Slide 13

Information Classification: General #BHEU  @BlackHatEvents


> Recovered by OCR — confidence 86/100 on the text kept, 45/100 across the whole page. Wording is approximate. Verify exact values against the source PDF.

```text
GuardianeLinks
YgayHoro Koga!
Ha GitHub yxe nc
```

## Slide 14

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 15

Information Classification: General #BHEU  @BlackHatEvents

## Slide 16

Information Classification: General #BHEU  @BlackHatEvents


> Recovered by OCR — confidence 79/100 on the text kept, 35/100 across the whole page. Wording is approximate. Verify exact values against the source PDF.

```text
Kak paGotaer OTkKpbITbIi peaMpeKr? \\r\\n\")" | gzip | base64°
```

## Slide 17

Information Classification: General #BHEU  @BlackHatEvents


> Recovered by OCR — confidence 79/100 on the text kept, 35/100 across the whole page. Wording is approximate. Verify exact values against the source PDF.

```text
Kak paGotaer OTkKpbITbIi peaMpeKr? \\r\\n\")" | gzip | base64°
```

## Slide 18

Information Classification: General

#BHEU  @BlackHatEvents


> Recovered by OCR — confidence 96/100 on the text kept, 95/100 across the whole page. Wording is approximate. Verify exact values against the source PDF.

```text
Trigona
Trigona ‘on Oct 01, 2023 12:47:34
1930
| is a leading global post-sales service support partner for original equipment
manufacturers (OEMs) across various industries, founded in 1982. Company offers services
supporting OEM customers through depot repairs, field services, supply-chain logistics, and
other professional services.
By acquiring this company's confidential data (including in full data), you will get
access to valuable information that can help you grow your business. You will learn about the
company's strategies, strengths, weaknesses, opportunities and threats. You will also discover
the needs, pain points, motivations and behaviors of its customers. You will be able to use this
information to create better products and services, target the right prospects, craft compelling
sales pitches and close more deals.
```

## Slide 19

Information Classification: General #BHEU  @BlackHatEvents


> Recovered by OCR — confidence 95/100 on the text kept, 81/100 across the whole page. Wording is approximate. Verify exact values against the source PDF.

```text
SAP exploits or vulnerabilities
800
600
400
Number of hits
200
2020 2021 2022 2023
Year
```

## Slide 20

#### Good and easy to understand indicator

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 21

Good and easy to understand indicator What threat actors speak about is not for fun…

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 22

Good and easy to understand indicator What threat actors speak about is not for fun…

… it is for profit

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 23

Information Classification: General #BHEU  @BlackHatEvents


> Recovered by OCR — confidence 95/100 on the text kept, 81/100 across the whole page. Wording is approximate. Verify exact values against the source PDF.

```text
SAP exploits or vulnerabilities
800
600
400
Number of hits
200
2020 2021 2022 2023
Year
```

## Slide 24

## What happened in 2021 ?

Information Classification: General

#BHEU  @BlackHatEvents


> Recovered by OCR — confidence 95/100 on the text kept, 78/100 across the whole page. Wording is approximate. Verify exact values against the source PDF.

```text
What happened in 2021 ?
400
Number of hits
200
2020 2021 2022
Year
2023
```

## Slide 25

#### RECON

SOLMAN RCE

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 26

RECON
SOLMAN RCE
#BHEU  @BlackHatEvents

Information Classification: General


> Recovered by OCR — confidence 96/100 on the text kept, 83/100 across the whole page. Wording is approximate. Verify exact values against the source PDF.

```text
SAP exploits or vulnerabilities
RECON
800
600
SOLMAN RCE
400
Number of hits
200
2020 2021 2022 2023
Year
```

## Slide 27

https://www.sygnia.co/threat-reports-and-advisories/elephant-beetle-an-organized-financial-theft-operation/ https://onapsis.com/blog/elephant-beetle-older-unpatched-sap-vulnerabilities-are-still-threat/

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 28

3 years ~ 30,000,000 USD

https://www.sygnia.co/threat-reports-and-advisories/elephant-beetle-an-organized-financial-theft-operation/ https://onapsis.com/blog/elephant-beetle-older-unpatched-sap-vulnerabilities-are-still-threat/

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 29

3 years
~ 30,000,000 USD

Information Classification: General

#BHEU  @BlackHatEvents


> Read by a vision model from the page image (replacing unreliable OCR) — confidence 96/100 on the text kept, 89/100 across the whole page. Wording is approximate. **This block contains dense hex, addresses or tabular data: individual values are frequently misread and its row/column structure is not preserved. Do not quote exact values from it — check the source PDF.**

```text
SAP exploits or vulnerabilities

3 years
~ 30,000,000 USD

Number of [exploits/vulnerabilities — axis label continues under the overlaid box, obscured]
800
400
200
0
2020    2021    2022    2023
Year
```

## Slide 30

#### Public exploits

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 31

#### Public exploits

FIN13 set a precedent

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 32

Public exploits FIN13 set a precedent Threat actors realized that SAP target could have great ROI

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 33

Public exploits Which companies / agencies FIN13 set **a** precedent lready run SAP ? Threat actors realized that SAP target could have great ROI

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 34

## Who are interested by SAP

Information Classification: General

#BHEU  @BlackHatEvents


> Recovered by OCR — confidence 83/100 on the text kept, 50/100 across the whole page. Wording is approximate. Verify exact values against the source PDF.

```text
sted by y SAP
} 15/24 yj No Upkeep
&
= |
17
19
Status:
```

## Slide 35

Name Particularity
FIN13 Financial companies target
FIN7 Financial companies target

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 36

Name Particularity
FIN13 Financial companies target
FIN7 Financial companies target
Cobalt Spider Point Of Sales target

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 37

**Name Particularity** FIN13 Financial companies target

FIN7 Financial companies target Cobalt Spider Point Of Sales target

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 38

Name Particularity
FIN13 Financial companies target
FIN7 Financial companies target
Cobalt Spider Point Of Sales target
APT10 Mainly known for espionage

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 39

**Name Particularity** FIN13 Financial companies target FIN7 Financial companies target Cobalt Spider Point Of Sales target APT10 Mainly know for espionage

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 40

“Good day, ladies and gentlemen! I'll be brief. We managed to gain access to 4 networks of different companies through CVE-2020-6287 [RECON] and CVE-2020-6207 [SAP Solution Manager missing authentication], after googling I found out that one of them was like a restaurant chain, and the other was a telephone company[...] What to do about it?  Sell?  How much?  Encrypt? [...]”

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 41

“Good day, ladies and gentlemen! I'll be brief. We managed to gain access to 4 networks of different companies through CVE-2020-6287 [RECON] and CVE-2020-6207 [SAP Solution Manager missing authentication], **after googling I found out that** one of them was like a restaurant chain, and the other was a telephone company[...] What to do about it?  Sell?  How much?  Encrypt? [...]”

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 42

“Good day, ladies and gentlemen! I'll be brief. We managed to gain access to 4 networks of different companies through CVE-2020-6287 [RECON] and CVE-2020-6207 [SAP Solution Manager missing authentication], **after googling I found out that** one of them was like a restaurant chain, and the other was a telephone company[...] **What to do about it?  Sell?  How much?  Encrypt?** [...]”

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 43

Name Particularity
FIN13 Financial companies target
FIN7 Financial companies target
Cobalt Spider Point Of Sales target
APT10 Mainly know for espionage
Script Kiddies Various threats

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 44

Name Particularity
FIN13 Financial companies target
FIN7 Financial companies target
   What can we learn from this result ?
Cobalt Spider Point Of Sales target
APT10 Mainly know for espionage
Script Kiddies Various threats

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 45

Name Particularity
FIN13 Financial companies target
FIN7 Financial companies target
Why are they interested ?
Cobalt Spider Point Of Sales target
APT10 Mainly know for espionage
Script Kiddies Various threats

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 46

Name Particularity
FIN13 Financial companies target
FIN7 Financial companies target
Cobalt Spider Point Of Sales target
APT10 Mainly know for espionage
   Because of what SAP processes… $$
Script Kiddies Various threats

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 47

Name Particularity
FIN13 Financial companies target
FIN7    Because of what SAP holdsFinancial companies target
Cobalt Spider Point Of Sales target
APT10 Mainly know for espionage
Script Kiddies Various threats

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 48

Name Particularity
FIN13 Financial companies target
FIN7 Financial companies target
     Because of public exploitsCobalt Spider Point Of Sales target
APT10 Mainly know for espionage
Script Kiddies Various threats

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 49

Name Particularity
Highlight wide range of SAP componen sFIN13 Financial companies targe t
FIN7 Financial companies target
Cobalt Spider Point Of Sales target
APT10 Mainly know for espionage
Script Kiddies Various threats

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 50

**Name Particularity** FIN13 Financial companies target FIN7 Financial companies target Cobalt Spider Point Of Sales target APT10 Mainly know for espionage Script Kiddies Various threats

https://en.wikipedia.org/wiki/List_of_SAP_products

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 51

#### Groups are very heterogeneous

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 52

Groups are very heterogeneous SAP target is not reserved individually to Hacktivist Nation State Criminal

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 53

Groups are very heterogeneous SAP target is not reserved individually to Hacktivist Nation State Criminal

SAP is so huge, that it has interest for all groups

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 54

## Price inflation

Information Classification: General

#BHEU  @BlackHatEvents


> Recovered by OCR — confidence 80/100 on the text kept, 51/100 across the whole page. Wording is approximate. Verify exact values against the source PDF.

```text
EUROPE 2024 ° AX
Strength:
Mm) 17
Intelligence:
[285/285 _|
```

## Slide 55

25,000 USD

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 56

50,000 USD

Information Classification: General

#BHEU  @BlackHatEvents


> Recovered by OCR — confidence 95/100 on the text kept, 74/100 across the whole page. Wording is approximate. Verify exact values against the source PDF.

```text
Target:
Bounty:
Start Date:
End Date:
90,000 USD
```

## Slide 57

50,000 USD

Information Classification: General

#BHEU  @BlackHatEvents


> Recovered by OCR — confidence 88/100 on the text kept, 85/100 across the whole page. Wording is approximate. Verify exact values against the source PDF.

```text
Target: SAP NetWeaver
Bounty: Up to!
Start Date:
End Date:
} for pre-authentication RCEs or authentication bypass exploits affecting recent v ons of SAP NetWeaver
jolt Should allow either remote code execution authentic 1 DYp work with default installations and should
fh of user interaction
90,000 USD
```

## Slide 58

250,000 USD

Information Classification: General

#BHEU  @BlackHatEvents


> Recovered by OCR — confidence 88/100 on the text kept, 86/100 across the whole page. Wording is approximate. Verify exact values against the source PDF.

```text
EUROPE 2024 mE
(7X Ow d f e n S e HOME BOUNTIES
Exploit Acquisition Program
PLM and EPR
e SAP (RCE): 250k USD
e Siemens Teamcenter (RCE): 250k USD
e Oracle ERP (RCE): 200k USD 250.000 USD
```

## Slide 59

Information Classification: General #BHEU  @BlackHatEvents


> Recovered by OCR — confidence 92/100 on the text kept, 89/100 across the whole page. Wording is approximate. Verify exact values against the source PDF.

```text
SSD Secure Disclosure @
Found a vulnerability in SAP NetWeaver?
Submit your findings today at ssd
get the BIG payouts you deserve @@& 6
Netweaver
exploits
are now in high-demand!
ssd-disclosure.com
113
```

## Slide 60

Information Classification: General #BHEU  @BlackHatEvents


> Recovered by OCR — confidence 92/100 on the text kept, 90/100 across the whole page. Wording is approximate. Verify exact values against the source PDF.

```text
SSD Secure Disclosure @
Found a vulnerability in SAP NetWeaver?
Submit your findings today at ssd-disclosure.com/sap-netweave!
get the BIG payouts you deserve @@& 6
Netweaver
exploits
are now in high-demand!
ssd-disclosure.com
113
```

## Slide 61

“Why do you still report your findings to SAP instead of selling them to me !!??”

Around event in 2021

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 62

~~“Why do you still report your findings to SAP instead of selling them to me !!?? ”~~

Around event in 2021

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 63

What do these prices tell us ?

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 64

#### Confident of what they can get back

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 65

Confident of what they can get back

Pay more to win time Finding SAP vulnerabilities… takes time Creating reliable exploit… takes time

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 66

Confident of what they can get back Pay more to win time Finding SAP vulnerabilities… takes time Creating reliable exploit… takes time Public critical exploits are 4 years old →Exploit lose their effectiveness →Seek for “fresh” weapons

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 67

## Takeaways

Information Classification: General

#BHEU  @BlackHatEvents


> Recovered by OCR — confidence 73/100 on the text kept, 47/100 across the whole page. Wording is approximate. Verify exact values against the source PDF.

```text
yt @ 2445 f 460 15/24 { NoUpkeep
End Game Options |
Restart Mission lr ry
Exit Game ] a
|
’ ys \ Level 1 Archmage 1] rail
‘\ 7 >) Armo 17
ve 4 3 Intelligence: f
"395 / 450 f nai
tr 285 /285 zl Status:
```

## Slide 68

#### SAP Interest is growing… From heterogeneous groups

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 69

SAP Interest is growing… From heterogeneous groups The demand for SAP 0day is growing... Because of ROI

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 70

SAP is no longer a black box → Consider SAP applications targeted

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 71

SAP is no longer a black box → Consider SAP application targeted

No 0days found →Mainly public vulnerabilities

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 72

|**CVE**|**CVSS**|**Exploit**|**KEV / Active**|**Ransomware Risk**|**SAP Security Note**|
|---|---|---|---|---|---|
|CVE-2010-5326|10.0|Public|**Yes / ACTIVE**|High|1445998|
|CVE-2016-2386|9.8|Public|**Yes / ACTIVE**|High|2101079|
|CVE-2016-2388|5.3|Public|**Yes**|Low|2256846|
|CVE-2016-3976|7.5|Public|**Yes / ACTIVE**|High|2234971|
|CVE-2016-9563|6.5|Public|**Yes**|Low|2296909|
|CVE-2018-2380|6.6|Public|**Yes / ACTIVE**|High|2547431|
|CVE-2020-6207|10.0|Public|**Yes / ACTIVE**|Medium|2890213|
|CVE-2020-6287|10.0|Public|**Yes / ACTIVE**|Medium|2934135|
|CVE-2021-38163|9.9|Public|**Yes / ACTIVE**|Medium|3084487|
|CVE-2021-33690|9.9|Public|No /**ACTIVE**|High|3072955|
|CVE-2021-42063|8.8|Public|No /**ACTIVE**|Medium|3102769|
|CVE-2022-22536|10.0|Public|**Yes / ACTIVE**|Medium|3123396|

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 73

SAP is no longer a black box → Consider SAP application targeted

No 0days found →Mainly public vulnerabilities →Patch them all !

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 74

SAP is no longer a black box → Consider SAP application targeted No 0days found →Mainly public vulnerabilities →Patch them all !

Not only internet exposed vulnerabilities

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 75

SAP is no longer a black box → Consider SAP application targeted

No 0days found →Mainly public vulnerabilities →Patch them all !

Not only internet exposed vulnerabilities →SAP systems are widely connected →Manage SAP security globally

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 76

SAP is no longer a black box → Consider SAP application targeted No 0days found →Mainly public vulnerabilities →Patch them all ! Not only internet exposed vulnerabilities →SAP systems are widely connected →Manage SAP security globally

Information Classification: General

#BHEU  @BlackHatEvents

## Slide 77

# Thank You!

**Yvan Genuer** info@onapsis.com

#BHEU   @BlackHatEvents

## Slide 78

##### THIS DOCUMENT CONTAINS **CONFIDENTIAL INFORMATION & IS SUBJECT TO THE CONFIDENTIALITY AGREEMENT BETWEEN THE PARTIES**

All data submitted in this document is solely for the use of the recipient and cannot be disclosed to any third party or used by the recipient for any other purpose than cooperation with Onapsis. This data shall not be duplicated, used or disclosed in whole or in part for any purpose. If a contract is awarded to Onapsis as a result of, or in connection with the submission of this data, the customer or prospective customer shall have the right to duplicate, use or disclose this data to the extent provided in the contract. This restriction does not limit the customer’s or prospective customer’s right to use the information contained in the data if it is obtained from another source without restriction. The data contained in this presentation does not constitute legal advice and is for informational purposes only. An attorney-client relationship is not presumed or intended by receipt or review of this presentation. Onapsis does not make any representations or warranties, express or implied, with respect to the accuracy or completeness of the data included in this presentation. Onapsis will have no liability with respect to the use or reliance upon this data by recipient. The following information is being shared in order to outline some of Onapsis’ current product plans, but it is important to understand that it is being shared for informational purposes only, and not as a binding commitment. Please do not rely on this information in making purchasing decisions because Onapsis makes no representations about future functionality, and ultimately, the development, release, and timing of any products, features or functionality remains at the sole discretion of Onapsis and is subject to change.

#BHEU   @BlackHatEvents
