...
Web SDK or adds the
reference to it|~160 + ~150
ASP.NET
Core|Largest assembly count
but still lacks useful
gadgets.|
|Microsoft.WindowsDesktop.App|application is a desktop|~160 + ~45|The rich row. Carries|
||application (UseWPF /|Win|many of the same .NET|
...
|`1`
`Uri`
`tex`|`Holder uriFromUriContext = TypeConverterHelper.GetUriFromUriContext(context,`
`t);`|
|---|---|
|`2`
...
- `5 { 6` **`return new`** `Cursor(resolvedUri.LocalPath); 7 } 8 WebRequest request = WpfWebRequestHelper.CreateRequest(resolvedUri); 9 WpfWebRequestHelper.ConfigCachePolicy(request,` **`false`** `);`
...
`3`
`4`
`5`|_`// Both arguments are attacker-controlled`_
...
|---|---|
|`6`
`7`|_`// Step 2: find a static Parse(string), public or not`_
`MethodInfo parse = targetType.GetMethod("Parse", BindingFlags.Public |`
...
`5`
`6`|_`// Both arguments are attacker-controlled`_
`object InsecureConstructorTransformer(string inputTargetTypeName, string`
`inputString)`
`{`
 _`// Step 1: attacker chooses the type`_
`Type targetType = Type.GetType(inputTargetTypeName);`|
...
`8`
`9`
`10`|_`// Step 2: construct it from the attacker string,`_
 _`//         matching the single-argument string constructor`_
 **`return`**`Activator.CreateInstance(targetType,`**`new`**`object[] { inputString });`
`}`|
...
|---|
|`1`
_`// A type whose constructor treats the string as a path and loads it`_
`2`
**`class`** `Plugin`
`3`
`{`
...
`{`
`6`
 _`// The string is loaded as an assembly during construction`_
`7`
...
**Listing 26.  C#** ResourceSet(string) opens the path and reaches BinaryFormatter `1` _`// System.Resources.ResourceSet`_ `2` **`public`** `ResourceSet(string fileName) :` **`this`** `() 3 { 4` **`this`** `.Reader =` **`new`** `ResourceReader(fileName); 5` **`this`** `.ReadResources(); 6 } 7 8` _`// System.Resources.ResourceReader`_ `9` **`public`** `ResourceReader(string fileName) 10 { 11` _`/*...*/`_ `12` **`this`** `._store =` **`new`** `BinaryReader(` `13` **`new`** `FileStream(fileName, FileMode.Open, FileAccess.Read, FileShare.Read, 4096, FileOptions.RandomAccess),` `14 Encoding.UTF8); 15` **`this`** `.ReadResources(); 16 }`
...
12internal StreamedWorkflowDefinitionContext(string workflowDefinitionPath, string
...
FileAccess.Read);
16/*...*/
17this.workflowDefinition = new byte[fileStream.Length];
...
Workflow association config carrying the attacker type
...
ProxyRequestResponse<T> carries the type argument
...
- `1 source = a Type resolved` **`from`** `input      // carried over` **`from`** `the shared hunt 2 sink   = Activator.CreateInstance(type) followed by reflective property 3 assignment (PropertyInfo.SetValue)` **`where`** `names/values are input-driven 4 report any flow(source -> sink) 5 6` **`from`** `Call create, Call setValue 7` **`where`** `create is Activator.CreateInstance on an input-driven type 8` **`and`** `setValue is PropertyInfo.SetValue on the created instance 9` **`and`** `taintReaches(anInput(), setValue.propertyNameOrValue())`
...
- `1 [ ] Can the attacker control the type being resolved? 2 [ ] Can the attacker control the string value being converted?` `3 [ ] Is there a type allowlist, or any validation, between resolution and conversion?`

- `4 [ ] Which assemblies can the target process load? 5 [ ] Is a working code-execution gadget among those assemblies 6 (XamlReader, ObjectDataProvider, AssemblyInstaller, ResXFileRef, ...)? 7 [ ] Does the path cross a trust boundary, and is it reachable unauthenticated?`
...
- `1` _`// input-driven type: the whole class in one line`_ `2 var obj = Convert(Type.GetType(input.TypeName), input.Value); 3 4` _`// fixed type: nothing to steer`_ `5 var address = ParseAddress(input.Value);` _`// destination is CustomerAddress, in code`_
...
Everything downstream, the converter, the `Parse` , the constructor, the accessors, is safe

now, not because those mechanisms changed, but because the type they act on can only be one the application chose.
...
`6`
`7`
`8`|**`static`** **`readonly`**`HashSet<string> AllowedPrimitives =`**`new`**`()`
`{`
 `"System.Int32", "System.Int64", "System.Double",`
 `"System.Boolean", "System.DateTime", "System.Guid", "System.String",`
`};`
`Type ResolvePermitted(string typeName)`
`{`|
|---|---|
|`9`
`10`
`11`
`12`|_`// fixed framework types: cleared by name, before any resolution`_
 **`if`**`(AllowedPrimitives.Contains(typeName))`
...
`18`
`19`
`20`|_`// the application's own opt-in types: resolve within our own assemblies`_
 _`// only (never the whole load space), then require the marker`_
`Type candidate = ResolveInOwnAssemblies(typeName);`
 **`if`**`(candidate !=`**`null`**`&&`
**`typeof`**`(IPermittedConversion).IsAssignableFrom(candidate))`
 **`return`**`candidate;`
 **`throw`** **`new`**`SecurityException($"Type not permitted: {typeName}");`
`}`|
...
- [16] Microsoft. "XmlConvert Class." <u>`https://learn.microsoft.com/en-us/dotnet/api/system.xml.xmlconvert` https://learn.microsoft.com/en-us/dotnet/desktop/xaml-services/type-converters-overview</u>
