...
2 Type.GetType("System.Windows.Data.ObjectDataProvider, PresentationFramework, Version=4.0.0.0, Culture=neutral, PublicKeyToken=31bf3856ad364e35");
```
...
2 Type.GetType("System.Windows.Data.ObjectDataProvider, PresentationFramework, Version=8.0.0.0, Culture=neutral, PublicKeyToken=31bf3856ad364e35");
...
15         result = Activator.CreateInstance(type, BindingFlags.Instance | BindingFlags.Public | BindingFlags.CreateInstance, null, new object[] { memoryStream }, null);
16     }
17     return result;
18 }
```
...
20 {
21     BinaryFormatter binaryFormatter = new BinaryFormatter(null, new StreamingContext(StreamingContextStates.File | StreamingContextStates.Persistence));
22     this._typeLimitingBinder = new ResourceReader.TypeLimitingDeserializationBinder();
...
```
19     this._store.BaseStream.Seek(this._dataSectionOffset + (long)pos, SeekOrigin.Begin);
20     typeCode = (ResourceTypeCode)this._store.Read7BitEncodedInt();
21     /*...*/
22     if (typeCode < ResourceTypeCode.StartOfUserTypes)
23     {
24         throw new BadImageFormatException(Environment.GetResourceString("BadImageFormat_TypeMismatch"));
25     }
...
```
1  <data name="BinaryFormatter_Payload" mimetype="application/x-microsoft.net.object.binary.base64">
```
...
3      : this(new StreamedWorkflowDefinitionContext(workflowDefinitionStream, null, null))
4  {
5  }
```
...
```
1  // System.ServiceModel.Description.WorkflowServiceBehavior
2  internal WorkflowServiceBehavior(WorkflowDefinitionContext workflowDefinitionContext)
3  {
4      /*...*/
5      this.name = this.workflowDefinitionContext.WorkflowName;
6      /*...*/
7  }
```

Reading `WorkflowName` forces the definition to be built. The getter calls `GetWorkflowDefinition` , which calls `DeSerizalizeDefinition` , which runs the XOML deserializer over the bytes:

**Listing 15. C#** DeSerizalizeDefinition runs the XOML deserializer

```
1  // System.Workflow.Runtime.StreamedWorkflowDefinitionContext
2  private Activity DeSerizalizeDefinition(byte[] workflowDefinition, byte[] ruleDefinition)
3  {
4      /*...*/
...
```
7      activity = new WorkflowMarkupSerializer().Deserialize(serializationManager, reader) as Activity;
8      /*...*/
9  }
```
...
```
1 UriHolder uriFromUriContext = TypeConverterHelper.GetUriFromUriContext(context, text);
...
```
1 UriHolder uriFromUriContext = TypeConverterHelper.GetUriFromUriContext(context, value);
...
7      MethodInfo parse = targetType.GetMethod("Parse", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static, null, new[] { typeof(string) }, null);
8      // Step 3: invoke it with the attacker string
9      return parse.Invoke(null, new object[] { inputString });
10 }
```
...
2  {
3      return XamlReader.Parse(xamlText, false);
4  }
5
6  public static object Parse(string xamlText, bool useRestrictiveXamlReader)
7  {
8      return XamlReader.Load(XmlReader.Create(new StringReader(xamlText)),
9                             useRestrictiveXamlReader);
10 }
```
...
5          return XamlServices.Load(new XamlXmlReader(xmlReader));
6  }
```
...
```
1  // Both arguments are attacker-controlled
2  object InsecureConstructorTransformer(string inputTargetTypeName, string inputString)
3  {
4      // Step 1: attacker chooses the type
5      Type targetType = Type.GetType(inputTargetTypeName);
6
...
13         new FileStream(fileName, FileMode.Open, FileAccess.Read, FileShare.Read, 4096, FileOptions.RandomAccess),
14         Encoding.UTF8);
15     this.ReadResources();
16 }
```
...
4
5  public WorkflowServiceBehavior(string workflowDefinitionPath, string ruleDefinitionPath)
6      : this(new StreamedWorkflowDefinitionContext(workflowDefinitionPath, ruleDefinitionPath, null))
7  {
8      /*...*/
9  }
10
11 // System.Workflow.Runtime.StreamedWorkflowDefinitionContext
12 internal StreamedWorkflowDefinitionContext(string workflowDefinitionPath, string ruleDefinitionPath, ITypeProvider typeProvider)
13 {
14     /*...*/
15     fileStream = new FileStream(workflowDefinitionPath, FileMode.Open, FileAccess.Read);
16     /*...*/
...
2  object InsecurePropertyTransformer(string inputTargetTypeName, IDictionary<string, string> inputProperties)
3  {
...
6          base.Refresh();       // -> BeginQuery -> QueryWorker -> InvokeMethodOnInstance
7  }
8
...
2  public override object ConvertXmlToObject(XmlReader xmlReader, XmlRootAttribute xmlAttrib)
3  {
4      // ...
5      XmlSerializer xmlSerializer = ObjectStorage.GetXmlSerializer(this.DataType, xmlAttrib);
...
9    this.EvaluateParameters(this.InsertParameters, ref baseTemplateId, ref associatedTemplateId, ref listId, ref itemId, ref empty);
10   this.SetWeb(empty);
11   SPListItem item;
12   SPWorkflowAssociation workflowAssociation = this.GetWorkflowAssociation(associatedTemplateId, baseTemplateId, listId, itemId, out item);
13   /*...*/
...
18     using (XmlTextReader xmlTextReader = new XmlTextReader(new StringReader(text)))
19     {
...
52           if (text2 != null)
53           {
54             if (typeConverter != null && typeConverter.CanConvertFrom(typeof(string)))
55             {
...
64               else
65               {
66                 hashtable[attribute] = TypeDescriptor.GetConverter(type).ConvertFromString(null, workflowAssociation.ParentWeb.Locale, text2);
67                 /*...*/
68 }
```
...
2  public static object ObjectFromString(Type objType, MemberInfo propertyInfo, string value)
3  {
```
...
11     {
12       pd = TypeDescriptor.GetProperties(propertyInfo.ReflectedType)[propertyInfo.Name];
13     }
14     if (pd != null)
15     {
...
30     MethodInfo methodInfo = objType.GetMethod("Parse", s_parseMethodTypesWithSOP);
...
6                             xmlns:Diag="clr-namespace:System.Diagnostics;assembly=system"
7                             FileName="Powershell"
8                             Arguments="calc"/>
9      <Diag:Process x:Key="proc01"
10                    xmlns:Diag="clr-namespace:System.Diagnostics;assembly=system"
11                    StartInfo="{StaticResource psi01}"/>
12     <ODP:ObjectDataProvider x:Key="odp01"
13                             xmlns:ODP="clr-namespace:System.Windows.Data;assembly=PresentationFramework"
14                             xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
15                             ObjectInstance="{StaticResource proc01}"
16                             MethodName="Start"/>
...
3  \.GetType\s*\(               # Type.GetType, Assembly.GetType, builder.GetType, ...
4  Assembly\.Load\w*\s*\(
5  Activator\.CreateInstance\s*\( # name-based overloads
6  \b\w*(Resolve|Create|Load)Type\w*\s*\(   # resolver / factory wrappers
```
...
```
1  # construction of a resolved type from arguments; read whether the type is input-driven
2  Activator\.CreateInstance\s*\(
3  GetConstructor\s*\([\s\S]*?\.Invoke\s*\(
```
...
3  [ ] Is there a type allowlist, or any validation, between resolution and conversion?
...
5  var address = ParseAddress(input.Value);   // destination is CustomerAddress, in code
```
...
2  {
3      readonly IReadOnlyDictionary<string, Type> _permitted;    // name -> Type, fixed at startup
4
...
9      {
10         if (!_permitted.TryGetValue(typeName, out var t))   // check the STRING first
...
16     if (candidate != null && typeof(IPermittedConversion).IsAssignableFrom(candidate))
17         return candidate;
18
...
- [1] Esser, S. "Shocking News in PHP Exploitation." POC, 2009. <u>`https://web.archive.org/web/20150523205411/https://www.owasp.org/images/f/f6/POC2009-ShockingNewsInPHPExploitation.pdf`</u>

- [2] Esser, S. "Utilizing Code Reuse/ROP in PHP Application Exploits." Black Hat USA, 2010. <u>`https://media.blackhat.com/bh-us-10/presentations/Esser/BlackHat-USA-2010-Esser-Utilizing-Code-Reuse-Or-Return-Oriented-Programming-In-PHP-Application-Exploits-slides.pdf`</u>

- [3] Forshaw, J. "Are You My Type?" Black Hat USA, 2012. <u>`https://media.blackhat.com/bh-us-12/Briefings/Forshaw/BH_US_12_Forshaw_Are_You_My_Type_WP.pdf`</u>
...
- [10] Mirosh, O. & Muñoz, A. "Room for Escape: Scribbling Outside the Lines of Template Security." Black Hat USA, 2020. <u>`https://i.blackhat.com/USA-20/Wednesday/us-20-Munoz-Room-For-Escape-Scribbling-Outside-The-Lines-Of-Template-Security-wp.pdf`</u>
...
- [16] Microsoft. "XmlConvert Class." <u>`https://learn.microsoft.com/en-us/dotnet/api/system.xml.xmlconvert`</u> <u>`https://learn.microsoft.com/en-us/dotnet/desktop/xaml-services/type-converters-overview`</u>
