---
title: "Security Through Transparency Scaling Your Customer Trust Program"
speakers: ["Ayoub Fandi"]
conference: "Black Hat"
conference_full: "Black Hat Europe 2023"
edition: "Europe"
year: 2023
source_pdf: "Black Hat Europe 2023 slides/Ayoub Fandi_Security Through Transparency Scaling Your Customer Trust Program.pdf"
pages: 28
sha256: "5c281013578875e9ff7e413f6cafef9457c410eec929da23251adb4346a1cc31"
text_chars: 5234
ocr_pages: 3
has_ocr: true
redacted_secrets: 0
ocr_confidence: 92.3
ocr_unreliable_blocks: 0
ocr_timeouts: 0
pages_recovered_from_text_layer: 0
companion_files: []
extractor: "pymupdf4llm 1.28.2 + tesseract"
converted_at: "2026-08-12T04:00:03Z"
---
# Security Through Transparency Scaling Your Customer Trust Program

**Speakers:** Ayoub Fandi  
**Conference:** Black Hat Europe 2023  
**Source:** `Black Hat Europe 2023 slides/Ayoub Fandi_Security Through Transparency Scaling Your Customer Trust Program.pdf` (28 pages)


## Slide 1

## Security through Transparency Scaling your Customer Trust Program

Ayoub Fandi - BlackHat Europe 2023 - 12/06/2023

#BHEU   @BlackHatEvents

## Slide 2

#### WHOAMI

- Ayoub Fandi, working in Customer Trust at GitLab

- Been working in the GRC vertical for about 5 years

- LinkedIn Learning course on GRC for the Cloud-Native Revolution

- awesome-security-GRC GitHub repo

- GRC Engineering Podcast

- I like GRC

#BHEU   @BlackHatEvents

Information Classification: General

## Slide 3

Why should you care about transparency…

#BHEU   @BlackHatEvents

Information Classification: General

## Slide 4

###### A story about Sales, security and hyperlinks

#BHEU   @BlackHatEvents

Information Classification: General

## Slide 5

###### Because it makes sense

- Transparency = good - Transparency = trust - Trust = good

#BHEU   @BlackHatEvents

Information Classification: General

## Slide 6

###### Effortless scaling

- Self service - Can focus on high-leverage work

#BHEU   @BlackHatEvents

Information Classification: General

## Slide 7

# Transparent with the public

#BHEU   @BlackHatEvents

Information Classification: General

## Slide 8

yes, we have the same security policies!

#BHEU   @BlackHatEvents

Information Classification: General

## Slide 9

Sharing is caring

- Leveraged by others to build their programs - Receive feedback to improve your own

#BHEU   @BlackHatEvents

Information Classification: General

## Slide 10

Open sourcing security

- Improving everyone’s security - Influencing the industry

#BHEU   @BlackHatEvents

Information Classification: General

## Slide 11

Getting started: Public-facing security wiki Share externally anything that wouldn’t have additional value to an attacker if public. Everyone knows you use AWS anyway.

#BHEU   @BlackHatEvents

Information Classification: General

## Slide 12

# Transparent with your customers

#BHEU   @BlackHatEvents

Information Classification: General

## Slide 13

##### Have you ever heard of security questionnaires?

#BHEU   @BlackHatEvents

Information Classification: General


> Recovered by OCR — confidence 95/100 on the text kept, 82/100 across the whole page. Wording is approximate. Verify exact values against the source PDF.

```text
black hat
Have you ever heard of security
questionnaires?
YOUR PROSPECT
SENDING
YOU A SECURITY
QUESTIONNAIRE
PROVIDING
Information Classification: Genera 1
```

## Slide 14

Increased exposure (not that type)

- Better reputation for your security team

- I can find your policies on Google

#BHEU   @BlackHatEvents

Information Classification: General

## Slide 15

###### Empowering with the right resources

- They have all the info they need

- - Maybe they’ll fill it themselves?

#BHEU   @BlackHatEvents

Information Classification: General

## Slide 16

###### Getting started: Sharing your security docs

#BHEU   @BlackHatEvents

Information Classification: General


> Recovered by OCR — confidence 89/100 on the text kept, 86/100 across the whole page. Wording is approximate. Verify exact values against the source PDF.

```text
black hat
AN Adobe About Adobe v Adobe Trust Center FocusAreas vy _ Product/Service Status By Solutions y — Resources We're committed to Information Security
It's our mission to be the leading example in security,
Compliance Ci .
innovation, and transparency.
Getting started:
Google Cloud Trust Center
Shari ity d
ari ng you r secu ri ocs How we focus on security, compliance, and privacy to earn the position of your most trusted cloud.
Explore security products Contact us
| Trust Center Privacy Compliance Products and services _ Industry Tools & Documentation
Responsible Al—learn more about our commitment to the advancement of Al driven by
SAP” » DocuSign Solutions» Products~ Plans & Pricing v CONTACT SALES BUY NOW TRY FOR FREE
_v / About SAP Trust Center Overview Alerts Compliance Legal Privacy Security SystemStatus Trust Portal
Security and Trust Overview — Cloud Status Security
Information Classification: General
```

## Slide 17

# Transparent with Sales

#BHEU   @BlackHatEvents

Information Classification: General

## Slide 18

###### This is us This is a chart This is sales

#BHEU   @BlackHatEvents

Information Classification: General


> Recovered by OCR — confidence 93/100 on the text kept, 82/100 across the whole page. Wording is approximate. Verify exact values against the source PDF.

```text
sublinear
(a)
This is us
&
Information Classification: General
linear
(b)
This is a chart
1% i
superlinear
(c)
This is sales
A
```

## Slide 19

###### Selling is hard, don’t make it worse

- Let’s be friends

- Let’s work together (free training)

#BHEU   @BlackHatEvents

Information Classification: General

## Slide 20

###### Smoothing out the cycle

- FASTER

- Less back-and-forths - Sometimes we’re not even involved

#BHEU   @BlackHatEvents

Information Classification: General

## Slide 21

###### Getting started: Database of security Q&As

- Are you secure? - Yes - Can you prove it? - Yes - Prove it then - I have ISO

#BHEU   @BlackHatEvents

Information Classification: General

## Slide 22

# Common objections

#BHEU   @BlackHatEvents

Information Classification: General

## Slide 23

I read “insecure by design” Everyone can get better at reading

#BHEU   @BlackHatEvents

Information Classification: General

## Slide 24

Looks like an impersonal way to sell The goal isn’t to sell, it’s to help sales. They are already doing the selling.

#BHEU   @BlackHatEvents

Information Classification: General

## Slide 25

How can I keep all of this up to date? You need to Git it right the first time.

#BHEU   @BlackHatEvents

Information Classification: General

## Slide 26

### AI

#BHEU   @BlackHatEvents

Information Classification: General

## Slide 27

You should try it. Let me know how it goes.

#BHEU   @BlackHatEvents

Information Classification: General

## Slide 28

# Any questions?

Feel free to reach out on LinkedIn (yes, I know, I don’t use Twitter) if you have any questions or need guidance on having a more transparent security program

#BHEU   @BlackHatEvents

Information Classification: General
