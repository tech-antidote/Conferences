---
title: "Cracking North Korea's Information Control How Smugglers, Defectors, and Technologists are Breaking Open the World's Most Locked-D"
speakers: ["JDT"]
conference: "DEF CON"
conference_full: "DEF CON 34"
edition: "34"
year: null
source_pdf: "DEF CON 34/DEF CON 34 - JDT - Cracking North Korea's Information Control How Smugglers, Defectors, and Technologists are Breaking Open the World's Most Locked-D.pdf"
pages: 47
sha256: "60e9cd990079564d9a0a8cd80fc2306950706c01c6c5bef6addd5a822da0b5ea"
text_chars: 10332
ocr_pages: 16
has_ocr: true
redacted_secrets: 0
companion_files: []
extractor: "pymupdf4llm 1.28.2 + tesseract"
converted_at: "2026-08-12T00:23:00Z"
---
# Cracking North Korea's Information Control How Smugglers, Defectors, and Technologists are Breaking Open the World's Most Locked-D

**Speakers:** JDT  
**Conference:** DEF CON 34  
**Source:** `DEF CON 34/DEF CON 34 - JDT - Cracking North Korea's Information Control How Smugglers, Defectors, and Technologists are Breaking Open the World's Most Locked-D.pdf` (47 pages)

## Slide 1

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
LIBER TY
in « DEFCQN
KOREA
```

## Slide 2

**Cracking North Korea’s Information Control**

## Slide 3

**I. INTRODUCTION Who I am and why this matters**

## Slide 4

**Jon Thompson** Chief Technology Officer Liberty in North Korea

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
REDACTED
Jon Thompson TZ
Chief Technology Officer
Liberty in North Korea
```

## Slide 5

## **Sysadmin > Threat Hunter > I ROOT NORTH KOREAN PHONES**

→ OSCP + SANS + MSc Cyber Security NK Human Rights

## Slide 6

### **WHY NORTH KOREA?**

North Korea has Citizens limited to a architected the national network that world’s most extreme is cut off from the info control system. global internet.

Sharing unauthorized information or media can result in torture or death.

## Slide 7

**II. THE SYSTEM** North Korea’s Full-Stack Information Control System

## Slide 8

providers connect North Korea to the **2** outside internet.

China Unicom + Russia’s TransTeleCom - Dual-homed since 2017

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
IBERTY
ORTH
COREA
providers connect North Korea to the
outside internet.
China Unicom + Russia’s TransTeleCom -
Dual-homed since 2017
```

## Slide 9

**The whole country routes through here.**

## Slide 10

**1,024**

Publicly routable IP addresses for the entire nation. 175.45.176.0/22 - less than a single mid-size company.

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
IBERTY
IN
|ORTH
OREA
1,024
Publicly routable IP addresses for the entire nation.
175.45.176.0/22 - less than a single aideiee company. |
SSF
```

## Slide 11

**Scanning North Korean servers**

## Slide 12

**24+ phone brands No app store**

Apps install only at state-run “IT Exchange Centers”

## Slide 13

### **The hardware**

Photos of a Pyongyang 2423 with its base model, the Blu Vivo XL3

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
IBERTY
IN
ORTH
OREA
The hardware
Photos of a Pyongyang 2423 with its base model,
the Blu Vivo XL3
```

## Slide 14

### **The hardware**

Photos of a Pyongyang 2421

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
The hardware
go
O°
ay j
Photos of a Pyongyang 2421
```

## Slide 15

### **The hardware**

Photos of a Han 701

## Slide 16

TRACEVIEWER / REDFLAG

**A screenshot every 5 minutes. Not deletable.**

Bundled into the Android OS on every phone - silent and persistent.

## Slide 17

# **The keyboard “autocorrects”**

**“comrade” “oppa” →**

South Korean style affection rewritten into ideology.

## Slide 18

**RedFlag: only state-signed files can spread**

**SELFSIGN** device signature

**NATISIGN** state signature

Unsigned files are auto-deleted on transfer - enforced at the OS level If the state didn’t sign it, it cannot move from phone to phone.

## Slide 19

**RedFlag: analyzing the signature**

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
RedFlag: analyzing the signature
0050ec40:
0050ec50:
0050ec60:
0050ec70:
0050ec80:
0050ec90:
0050eca0:
0050ecb0:
0050ecc0:
0050ecd0:
0050ece0:
0050ecf0:
0050ed00:
0050ed10:
0050ed20:
0050ed30:
0050ed40:
0050ed50:
0050ed60:
0050ed70:
0050ed80:
0050ed90:
00S0eda0:
0050edb0:
0050edc0:
0050edd0:
0050ede0:
0050ed£0:
6470
6e6E
0200
afS8
0000
2f£6d
6c61
0000
80ee
5706
9521
80ee
e4d4
Tice
7037
ed81
e359
7155
b8el
de92
755
ad2e
9764
115b
3e6c
le3f£
4b7c
le2ce
692E
726d
0200
0000
0000
6d6é9
7368
cb01
alaé
3al9
3ea0
alaé
4dtf7
946d
cae4
8aft7
5400
feea
5ite
195b
af1ié
180a
9631
£9a4
1£97
b46c
6564
daf10
646b
61l6c
0000
afS8
0000
6170
5£68
cb01
eba4
3bt4
73a8
724¢c
b332
ccef
2df1
£050
ec0c
2935
bdfe
cSab
11£9
03d0
27ee
0474
9fe7
052c
ibbe
4001
S£62
2e39
0000
0000
b8fc
2£73
2e6a
0597
ed20
78dS
fbba
57d5
800b
54865
5173
bd89
bf£53
8358
5377
59b0
03960
£89a
3edS
84b4
726c
bdb2
9644
0000
746e
2e70
ed70
2000
4£00
706c
7067
0000
lbed
8£38
89£7
9618
8c68
deb2
70dS
T3bft
9440
876d
OSla
8273
Saal
694f
Sdb8
6084
368f
bdi8
9458
4e41
S£62
6e67
4147
0000
6173
6173
504b
asss
70ee
0783
al92
65£5
Tibe
15e6
daSf£
de22
c997
a2fc
dbd7
acb6
25aa
Sb83
Gaba
95be
1c78
7063
86e8
5449
6l6e
504b
5£98
0000
7368S
682£
0506
5000
684b
£808
ebcl
073£
52b4
2e2a
2e4f
3031
b442
fa8b
e91c
6ad4
£0c£
2164
e27da
£3ef
63d£
ceal
déb0
5349
6bS£
0102
£41c
0000
7473
7370
0000
0000
3ac5S
1e92
adf3
21cd
9760
3ada
ef05
8105
bb52
a533
6788
62a6
6999
9e25
e4bf
7190
al0c
5835
9724
474e
dpi/dk_btn_bank_
normal. 9.pngPK. .
--O.assets
/mmiap/splash/sp
lash_h.jpgPK....
Tena ee re ae
- ..p-hK:.
qU..)5.X-m.
ove,sahv ce iS
vie
ae
led.
Pen |
002c9ad0: c55d
Theft
c55d
Thét
c55d
Thét
eSSd
Thét
cS5d
Thét
cS5d
Th6t
c55d
Thét
c55d
Thét
c55d
Thét
c55d
Th6t
002c9c10: c55d
002c9c2! Th6ft
002c9c3! ¢S5d
002c9c4! Teét
002c9c5: cS5d
002c9c6' Thét
002c9c7! 9503
002c9c80: d4fd
002c9c9' 474e
63ca
41ff
63ca
41ff
63ca
41ff
63ca
41ff
63ca
41fft
63ca
41ff
63ca
41ff
63ca
41ff
63ca
41fft
63ca
41ff
63ca
4iff
63ca
4i1ff
63ca
3309
6ddf
1803
c92a
S6fc
c92a
56fc
c92a
S6fc
c92a
56fc
c92a
S6fc
c92a
56fc
c92a
56fc
cS2a
56fc
c92a
56fc
c92a
56fc
c92a
56fc
c92a
56fc
c92a
a37e
5572
0000
d4e5
b0d2
d4e5
b0d2
d4e5
b0d2
d4e5
b0d2
d4e5
b0d2
d4e5
b0d2
d4e5
b0d2
d4e5
b0d2
d4e5
b0d2
d4e5
b0d2
d4e5
b0d2
d4e5
b0d2
d4e5
5374
9a53
0000
785b
a09a
785b
a09a
785b
a09a
785b
a09a
785b
a09a
785b
a09a
785b
a09a
785b
a09a
785b
a09a
785b
a09a
785b
a09a
785b
a09a
785b
038c
Sf£f£0
0000
£537
8946
£537
8946
£537
8946
£537
8946
£537
8946
£537
8946
£537
8946
£537
8946
£537
8946
£537
8946
£537
8946
£537
8946
£537
cd2c
126£
$345
al8c
525
al8c
525c¢
alBc
525c¢
al8c
525c¢
al8c
525c
alBc
S525c
al8c
$25c
al8c
S25c¢
al8c
S25c¢
al8c
S25c¢
al6c
S25c¢
al8c
S25c
al8c
8d9a
esft4
4c46
9bf4
4ed7
9bf4
4ed7
9bf4
4ed7
9bf4
4ed7
Sbf4
4ed7
Sbf4
4ed7
Sbf4
4ed7
Sbf4
4ed7
Sbf4
4ed7
9bf4
4ed7
Sbf4
4ea7
Sbf4
4ed7
Sbf4
9b81
4fa2
5349
i ae
---FR\N.
BPRes ve
--+FR\N.
FES
---FR\N.
—_
```

## Slide 20

#### **SELFSIGN: 792-byte cryptographic media signature**

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
SELFSIGN:
792-byte cryptographic media signature
per-file hash accumulator reserved
| IMEI hash | length | magic
RSA-2048 sig padding
256 B 32B 328 416B 40B 4B 4B 8B
0x000 0x100 0x120 0x140 Ox2E0 0x308 0x30C 0x310 0x318
```

## Slide 21

**NATISIGN: 320-byte cryptographic APK signature**

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
z5
8678
320-byte cryptographic APK signature
checksum
length
magic
RSA-2048 signature
256 B
extended
48B
4B
4B
8B
0x000
0x100
0x130
0x134
0x138
0x140
```

## Slide 22

# **Traceviewer + RedFlag Demo**

traceviewer_demo.mp4

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
[= rmstowe risa oon tener cemnetn =X F
wets =
Qe
a
J
.
4
>< a6
Galley Camera
traceviewer demo.mp4
```

## Slide 23

DESKTOP

**On desktop, it’s mostly split between Windows 7 and 10/11**

## Slide 24

**Exfil’d Data.**

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
IBERTY
ORTH
COREA
Data.
BuildNumber:
Caption: Micro 7 Professional
Operating system
Architecture: 32-bit
BuildNumber: 7600
Caption: Microsoft Windows 7 Professional
input Details (Input
Code Pages
ACP: 950
OEMCP: 950
Locale IDs (LCIDs)
System: 1028 (0x04
User: 1033 (0x0409)
Thread: 1028 (0x0404)
Keyboard / IME
Layout Handle (HKL): 0x0000
IME Description: t 9.10
IME File: UniKorHn.ime
=) Process
Input Details (Inputonapshot)
Code Page:
Keyboard / IME
Layout Handle (HKL):
IME Description: 3
IME File: UniKorHn.ime
(0.2000
Integrity Level: High (0x3000)
Collected at: 2026-04-28T07:
Internal Use Only
```

## Slide 25

**Hana IME**

## Slide 26

#### **Hana IME:**

**Get OS version and modify the registry**

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
IBERTY
ORTH
COREA
Get OS version and modify the registry
110
aa
112
113
114
115
116
i &)
118
119
120
121
122
123
124
125
126
127
128
pl ks,
130
131\}
HE Ul
}
FuN_10008010(
FUN_10008290 (r
plVar7 T 1014afcs;
uVaré = uVarlo;
uVar9 = uVarl0;
do {
if ({*(uint *) ({Longlong) DAT ldafcO + uVar9) & osMask) == osMask) { ——————— OS
DAT_10086e28 = (HKEY)*plVar7: i
Varé = {longlong) (int)uVa
WriteRecord (s SYSTE tC et ntrol 101l4aftd0 + lVar6é, «DAT 1014b0d0 + lVaré, <= Panel
(LPCWSTR) (&DAT_1014b1 + lVar6é),*{uint *) (({longlong) DAT _1014b154 + uVar3), :
*(DWORD *) ( (longlong) sDAT _1014b150 + uVar9));
uVarl0 = uVarl0 + 1;
uVars = (ulonglong) {(int)uVa
uVar9 = uVarld * 0
plVar7 =
} while ({sDAT afc) [uVarl0 * Ox75] != 0);
return 0;
```

## Slide 27

#### **Hana IME:**

#### **The hidden feature that no one asked for…**

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
IBERTY
IN
ORTH
OREA
Hana IME:
The hidden feature that no one asked for...
avivvaeve vu wii
oO 10155£40 Of ff 00 RegRecord
10155f40 Of ff 00 00 hana_dwordFFo Fh marker _ 10
10155f44 00 00 ff ff hana_dwordFFFFOO00h marker hi
10155f48 62 00 60 80 ff hana_qwordFFFFFFFFE0000002h hKey
10155£50 53 4£ 46 54 57 char[256] “SOFTWARE\\Microsoft\\... keyPath
10156050 45 6e 61 62 6c char[128] “EnableLUA” valueName
ase
1015€0d0 04 00 60 00 hana_dwordéh dataSize
101560d4 04 00 00 00 hana_dword4h dataType
ot 101560d8 00 00 00 00 00 hana_byt...””" data
```

## Slide 28

2020

**Group 109 cuts the power - then searches every room.**

Reactionary Ideology & Culture Rejection Act.

## Slide 29

MONITORING

**Bureau 27 hunts smuggled phones by their radio signals**

New detection methods can catch sub-minute calls

## Slide 30

**Distribution can mean the death penalty.**

Roving inspection units · warrantless raids · device seizures

## Slide 31

**III. THE CRACKS** How people beat the system anyway

## Slide 32

**Smuggled media**

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
IBERTY
ORTH
COREA
Smuggled
Sa ee
```

## Slide 33

**Trial and error**

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
IBERTY
IN
ORTH
OREA
Trial and
```

## Slide 34

**“Pigeon” Self-sign to beat the check.**

Self-signing media to defeat RedFlag’s signature enforcement.

## Slide 35

**Pigeon**

## Slide 36

**Contextual anti-forensic tools**

> Text below was recovered by OCR from an image-only slide; treat wording as approximate.

```text
Contextual anti-forensic
tools
Norton
Ghost
10.0
r Windows® XP Home/
>) symantec XP Pro/2000 Pro ONLY
10440175-IN
Product Activation Required
This CD may be used as an emergency boot disk.
Retrieve files, scan for viruses, check for errors, and restore
backup images with Symantec Recovery Disk. 256 MB RAM required.
Please see manual for instructions.
9/05 10440176-1N
```

## Slide 37

**IV. THE GAPS** The support system is collapsing

## Slide 38

**The biggest crisis for the movement since the 1990s.**

But a small community of defectors, activists, and technologists persists.

## Slide 39

**V. OUR WORK** What we’re doing about it

## Slide 40

**What we’ve built so far…**

**4**

Software projects in active development

**68**

Defector user tests in 2025

**SECURITY**

The primary design requirement in all of our products

## Slide 41

**VI. THE ASK** This is where you come in

## Slide 42

**If you write code, hack, or build - we need you.**

## Slide 43

#### **What you’d work on.**

**PHONE EXPLOITS**

Root, unlock, and exploit NK handsets

**EMBEDDED SYSTEMS**

Hardware analysis, application development, censorship bypass

**AI**

Scale analysis, future state, application development

##### **DIRECT-TO-CELL**

Reach devices independent of the state network

## Slide 44

Di$tribution Networks

# **The most resource-intensive part of our work**

Developing a single network can cost tens-of-thousands of dollars

## Slide 45

**“LiNK is developing technology to break barriers which I have no doubt will create a free world for North Koreans. I promise to contribute in whatever small way I can until the day when our dreams of a beautiful society and freedom in North Korea come true.” - Jang Hyeok, escaped NK in 2019**

## Slide 46

**volunteer.libertyinnorthkorea.org volunteers@libertyinnorthkorea.org**

**Scan to volunteer!**

## Slide 47

**THANK YOU**
